...
| Version-only override mislabels results | ⬜ open | 1 | `config.override({"iit.version": "IIT_4_0_2023"})` (or `version:` alone in `pyphi_config.yml`) is accepted, keeps the default measure, and labels a current-definition value (Fig 1A aB φₛ 0.0405, not 0.1719) as 2023; `IIT_3_0` alone already raises. Fix in `pyphi/conf/constraints.py` |
