...
| B22 raw-float φ-comparison audit | ⬜ open | 1 | Codebase-wide sweep for φ/Φ comparisons done as raw Python floats where they must be precision-aware (`utils.eq` / `PyPhiFloat` / `config.numerics.precision`). Same bug class as the S1 Composition escalation fix in `condensation.py`: two mirror-isomorphic candidates whose Φ differed by 5.6e-16 of summation noise resolved arbitrarily instead of tying. Audit every `==`/`<`/`>`/`min`/`max`/`sorted`/argmax/`set`/`dict`-key/`in` over a φ or Φ value; fix each to precision-aware comparison; add a regression guard. |
...
- **B22 — raw-float φ-comparison audit.** Sweep the codebase for φ/Φ comparisons performed on raw Python floats where the comparison decides an outcome (equality, ordering, deduplication, or selection) and so must respect `config.numerics.precision`. **Motivating bug (already fixed):** the S1 Composition escalation in `pyphi/condensation.py` compared big Φ as raw floats, so on the symmetric tie fixture two mirror-isomorphic candidates whose Φ differed by 5.6e-16 of summation noise resolved arbitrarily instead of tying; comparing `PyPhiFloat` at config precision fixed it (the tie substrate now correctly condenses to its symmetric three-singleton candidate at φ = 0.0845). The same pattern can hide anywhere a bare `==`/`<`/`>`/`min`/`max`/`sorted`/argmax, a `set`/`frozenset` membership or `dict` key, or an `in` test runs over a φ or Φ magnitude instead of routing through `utils.eq` / `utils.is_positive` / a `PyPhiFloat`. The precision-aware comparison surface already exists (`pyphi/utils.py`, `pyphi/data_structures/pyphi_float.py`, the `resolve_ties`/`lex_key` tie machinery) — the audit finds the call sites that bypass it. **Scope:** grep-and-read every φ/Φ comparison across `pyphi/` (condensation, MIP selection, purview/state tie-breaks, relations, macro grain search, AC α, CES distance), classify each as precision-safe or a latent tie/ordering bug, fix the unsafe ones to precision-aware comparison, and add a regression guard (a targeted test on a summation-noise tie fixture, and/or a lint against bare float comparison of the marked quantities). A confirmation experiment per suspected site (per CLAUDE.md "don't defer confirmation experiments") settles whether a given raw comparison is actually reachable with a sub-precision gap before it is called a bug. Sibling to the Complex-unification work that surfaced it.
