...
| N11 DBN export (lightweight) | ⬜ open | 6 | 2-timeslice DBN structure export (nodes_t→nodes_{t+1}, TPM→CPDs) as a plain networkx/dict for pgmpy/d-separation workflows downstream — **no pgmpy dependency** (honors the do-not-add-pgmpy rule). Pulled into 2.0 (lightweight scope; the full pgmpy integration stays out). |
