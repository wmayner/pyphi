...
| B12 `pyphi.Sweep` | ⬜ open | 6 | Cartesian batch driver (`over_states`/`over_subsets`/`over_formalisms`) → one tidy MultiIndex DataFrame, raw results in `.results`, per-row `ConfigSnapshot`/seed. Pulled into 2.0 (no blocker). |
| N7 `pyphi.analyze()` | ⬜ open | 6 | One high-level `analyze(substrate, state, formalism=…)` entry point. Pulled into 2.0 (no blocker). |
| N9 labeled-export audit | ⬜ open | 6 | Audit and close the unified `to_pandas`/xarray export story — largely delivered by P14d + B21 (`models/pandas.py` base + TPM/diff/explanation coverage); confirm full result-type coverage and fill gaps. Pulled into 2.0. |
