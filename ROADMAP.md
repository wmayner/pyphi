...
| N1 paper-reproduction suite | ✅ landed | 1 | Acceptance suite (`test/integration/test_paper_reproduction.py`) reproduces every published worked example with paper-sourced pins: **IIT 4.0 (2023) Figs 1/2/4, all of Fig 6 (A–E), all of Fig 7 (A/B/C)**; IIT 3.0 (2014) Fig 12; AC 2019 Fig 6; Gómez 2020 Fig 3A. The previously-blocking **Fig 6 A/B/D/E + Fig 7 weight matrices (graphical-only in the paper) were supplied by the authors** and ship as `examples.iit4_2023_fig6{a..e}_substrate` / `iit4_2023_fig7_substrate` / `iit4_2023_fig7_inactivated_substrate` (+ `*_system`); 2.0 reproduces the **published** φ_s/Φ/distinction-relation counts to 2-dp (6A bottleneck, 6B modular, 6C copy-ring, 6D specialized Φ=11452 via analytical relations, 6E exclusion; Fig 7 A/B with 13740/13111 relations, Φ=22.26/18.55; **Fig 7C** E-*inactivated* φ_s=0.11, 14 distinctions, 363 relations, Φ=3.35). **Fig 7C resolution:** "inactivation" is a *lesion* — E's state frozen into the dynamics (`fig7_substrate().factored_tpm.condition({4: 0})`), a **distinct** operation from a background-conditioned subsystem; built that way 2.0 reproduces the panel exactly. The discrepancy localized to mechanism {A} (the unit most coupled to E, weight 0.8) and was verified **byte-identical to the pre-refactor `b3aaa3e5`** (no regression). For **Fig 6**, the authors' *working notebooks* carry pre-final φ magnitudes differing from their own published figures (a scheme sweep over all 8 system schemes reproduces none); 2.0 matches the published *figures* — so only the notebooks' weights + structure are reused. |
