...
| Selection-margin reporting | ✅ landed | 7 | Margins at every IIT 4.0 selection. System level: `partition_margin` (normalized-φ gap to the best competing system partition; `None` when the sweep short-circuited — no false margins over truncated sweeps), per-direction specified-state margins, `tied_selections` naming the effectively tied selections, `effectively_tied`. Mechanism level: `RIA.partition_margin`, `MICE.purview_margin`, `state_margin`, and `effectively_tied` on RIA/MICE/`Distinction`. `shortcircuit_sia=False` now also disables the sweep-level early stop (system + mechanism), making margins exact everywhere with unchanged φ values. Surfaced via `explain()`, FULL-verbosity cards, `to_pandas`, serialization, relabel; consumed by `pyphi.sweep` rows (near-tied cell flagging) and `phi_posterior`'s margin-gated screening. IIT 3.0/AC deferred (different selection structure). Docs: tie-breaking how-to §Selection margins, theory §Selection margins. Spec: `2026-07-07-substrate-parameter-landscapes.md` §7 |
...
- **N23 — Φ-explanatory diagnostics.** *`2.x`.* *(Partially discharged: selection margins and the `pyphi.landscape` sections cover the sensitivity/landscape half; the determinism/degeneracy covariates remain.)* Surface the Marshall et al. 2023 system-φ
