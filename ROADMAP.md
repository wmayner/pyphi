...
| B19 CM/TPM consistency check | ⬜ open | 2 | Default-on `validate_connectivity` rejecting an under-specified CM (omits a TPM-implied edge → silent φ under-count); over-specification stays legal. Reuses `infer_edge`/`infer_cm`; sibling to B13. *(Added 2026-06-15.)* |
...
| B21 unified display model | ⬜ open | 2 | Structured description→multi-backend renderer (ASCII always-on, styled HTML for notebooks, optional lazy `rich`); replaces the `fmt.py` spaghetti + ad-hoc per-class reprs; full `_repr_html_` coverage; one `repr_verbosity` site. Foundational for B8/B15; pairs with P14d. *(Added 2026-06-15.)* |
...
- **B19 — CM/TPM connectivity-consistency validator (default-on).** *(Added 2026-06-15.)* A **directional** check run at substrate construction from the raw joint / state-by-node TPM + CM: infer the true edge set (an edge `A→B` exists iff `P(B | C, A=0) ≠ P(B | C, A=1)` for some context `C` — the test `infer_edge` / `infer_cm` in `core/tpm/joint_distribution.py:467-509` already implements it) and reject only an **under-specified** CM (one that omits a real edge, silently marginalizing a true dependency in `get_inputs_from_cm` / `irreducible_purviews` and under-counting φ). **Over-specification stays legal** — a CM that declares unused edges is a conservative, slower search, never wrong. Adds `validate_connectivity: bool = True` to `conf/infrastructure.py` (joining the `validate_*` family, lines 99-143), wired into `validate.substrate()` (`validate.py:78-94`), raising and naming the offending edge(s); default-on is a breaking-ish 2.0 change (substrates that previously ran with a permissive CM now raise), `validate_connectivity=False` as opt-out. Only meaningful when a joint TPM is supplied alongside a CM (a per-node / factored TPM is consistent by construction). Port the legacy `JointTPM`-only `infer_*` to operate on the construction-time input (or `FactoredTPM`). Cost `O(N²·2^(N-1))` — same order as materializing the TPM, negligible vs φ. Sibling to B13; advances "make illegal states unrepresentable." Pre-freeze (adds a config flag).
