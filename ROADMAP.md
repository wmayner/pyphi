...
- **ii-gated grain scheduling** *(open build; 2026-cap arm only)*. The GID arm
  ...
  heuristic and must be labeled approximate. *Prerequisite landed
  (2026-07-11):* the formalism-default decoupling (dashboard row) pins every
  φ-asserting test to its formalism, so the default can move to the 2026 cap
  under which the certified prune is sound. The gate build itself is still
  open.
