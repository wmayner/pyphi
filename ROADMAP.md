...
- **p53-Mdm2 golden — gated on k-ary EMD (2026-06-14); network reconstructed & verified.** The Gómez et al. **2020** ("multi-valued elements") p53-Mdm2 network was fully reconstructed from **Table 3** (the paper's "Table 1" reference is to the *data-class* table; the evolution function is Table 3) and **verified** — all 12 multi-valued transitions reproduce and the evaluated state (P,Mc,Mn)=(0,0,1) is the paper's fixed point. State order **P (ternary), Mc, Mn (binary)**; per-node deterministic functions `P'=2·(1−Mn)` (P←Mn), `Mc'=[P==2]` (Mc←P), `Mn'=Mc ∨ (P==0)` (Mn←P,Mc); cm (order P,Mc,Mn) rows `[[0,1,1],[0,0,1],[1,0,0]]`; built via `Substrate(marginals=[...], state_space=((0,1,2),(0,1),(0,1)))`. **What's needed — k-ary EMD (a scoped extension, not a port).** Non-binary IIT 3.0 *did* land at the pipeline level (P12: the SIA/CES machinery runs on k-ary substrates — a (3,2,2) `sia()` reaches the measure call under the `iit3` preset). The paper's `Φ(001)=0.44` (3 mechanisms) was computed with `MEASURE='EMD'` (stated explicitly in the paper, via their *extended* PyPhi), and **EMD is the only IIT-3.0 measure still binary-only**: `measures/distribution.py` registers `EMD` with `supports_alphabet=_binary_only`, and its kernels hard-code a `2^N` Hamming ground matrix (`hamming_emd`/`_hamming_matrix`) and a binary effect-EMD shortcut (`effect_emd`, `|P(node=0)| diff`) — the `# TODO extend to nonbinary nodes` at `distribution.py:620`. Both generalize cleanly: the cause-EMD ground matrix → Hamming distances over the actual product state-space (count of differing k-ary positions); the effect-EMD per-node term → total-variation between k-ary marginals. (My first-pass "no k-ary measure / needs a branch port / out of 2.0" read was **wrong** — only EMD's ground metric is missing; the other IIT-3.0-compatible measures default to `_any_alphabet`. A separate minor inconsistency: `compatible_measures` uses long names, e.g. `ABSOLUTE_INTRINSIC_DIFFERENCE`, while the registries use short keys, e.g. `AID`.) Once k-ary EMD lands, the reconstructed network reproduces 0.44 immediately and lands as a further k>2 golden. *Computable-today alternative:* the **binarized** panels — Fauré-Kaji `Φ(0001)=0.51` (4 mechs) and Tonello `Φ(0001)=0.71` (12 mechs) — are 4-node *binary* nets (Table 3's binary columns, with Van Ham "=" substitutions) and run under binary IIT 3.0 EMD; they reproduce a binarization, not the multi-valued original.
