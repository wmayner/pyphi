...
| B18 adaptive chunking | ⬜ open | 4 | Cost-model bin-packing for parallel granularity (activates the dormant `size_func`; guarded by N2). **Unblocked** — the parallel-engine unification landed; cost-balanced LPT packing + the ROADMAP cost functions drop onto the single path |
