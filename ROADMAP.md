...
| P11 cluster backends | 🟡 partial | 4 | Dask backend landed 2026-07-20: `DaskScheduler` distributes map-reduce over a user-connected `distributed.Client` (snapshot propagation, cost-balanced chunks, deterministic short-circuit prefixes; nested dispatch runs in-task); `cluster` extra; `docs/howto/chtc.md` covers CHTC (plain condor sweeps + fat-node fully supported; Dask worker pools documented as a pilot pending CHTC port-access confirmation — most ports on CHTC nodes are closed, their first-party Dask/htmap glue is dead/archived, so the guide lists the facilitation questions). Remaining: HTCondor-native campaign surface (assume Dask infeasible on CHTC until the port answer says otherwise) — tasks-as-data, not closures: prepare a job directory (specs serialized via `pyphi.serialize`, fixed runner entry point, generated submit file), `condor_submit` by the user, collect/resubmit from per-task output files; no long-lived coordinator. Task types: sweep cells (substrate axis) → `SweepResult`, and CES shards for one system (mechanism ranges, plus purview-range and partition-range sharding for heavy mechanisms — merges re-resolve ties; partition-range sharding needs stride-sliced lazy enumeration, `_find_mip_single_state` currently materializes the partition list). **Scope is user-declared and first-class**: the combinatorially feasible surface specified declaratively per axis (explicit lists, order bounds, structural predicates like unit-containment — data, not lambdas, so specs ship to jobs and land in provenance); `estimate_analysis` does scope-aware admission control and bin-packing at prepare time. Feasibility math (2026-07-20): under `JOINT_PARTITION_ALL` the all-units mechanism's (n,n) sweep is ~10⁹·⁷ at n=8, ~10¹³·⁵ at n=10, ~10⁴³ at n=21 — sharding reaches n≈10-11 dense; beyond that the campaign must shape workload (order caps + certified bounds) and, for sparse substrates, detect reducibility fast via bottleneck-first partition ordering (the sweep already short-circuits on the first zero-φ partition). Partial-CES relations are sound: verified 2026-07-20 (fig4, 3-of-4 subset) — relations depend only on their relata; analytical == concrete on subsets == full structure restricted to the subset, so partial structures give exact relation substructures and certified Σφ_r lower bounds (`sum_phi_relations_measured_bound`). |
