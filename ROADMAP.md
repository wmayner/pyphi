...
| B5 cross-formalism diff testing | 🟡 partial | 1 | Core invariants landed (`2026≤2023` — caught+fixed an Eq-23 cap MIP-selection bug; "3.0/4.0 agree on reducibility" **refuted**; **`b3aaa3e5` 4.0 non-regression oracle landed** — 4.0 `signed_phi` reproduces the pre-refactor raw φ to machine epsilon, clamped φ = `max(0,raw)`). Remaining: AC/IIT sign invariant |
