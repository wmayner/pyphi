...
P0 · P1 · P2 · P3 · P4 · P5 · P6 · **P6a** (free-threaded 3.14t CI lane + content-cache thread-safety: eviction/refcount bookkeeping locked, hot path lock-free) · **P6b** (graphillion removed; pure-Python relations enumeration) · P7 · **P8** (incl. `PhiFold`) · P9 · P10 · P10b · P10c · **P11 core** (Scheduler Protocol + Process/Thread schedulers + cost-sampling chunking + config-snapshot propagation) · P11.5 · P11.6 · P11.7 · **P11.8 Tier 1** · P11.9 · P11.85 · P11.86 · P11.87 · P11.88 · P11.95a · P11.95b · P11.95d · P11.95e · **P12** (factored TPM + multivalued units, incl. AC k-ary) · P14 · **P14b core** (sub-projects 1–4) · P14d (sub-projects B + A-1..A-4) · **Macro / Marshall-2024 intrinsic units** (SP1 + SP2 + SP3 + search parallelization) · **N2** (parallel≡sequential invariant) · **P11 loky-flake** (root-caused to the P9 cache-registry leak; confirmed resolved, N2 guards it) · **B1** (runtime bound-certificate assertions; `validate_phi_bounds`, on suite-wide) · **P13 SP2** (bite-rate study: bounds don't prune in the certified domain → no pruning built, SP1 is the deliverable) · **Eq-23 cap MIP-selection fix** (cap no longer shifts the system MIP; 2026 φ ≤ 2023 φ restored — found via B5) · **B9** (decimal precision oracle: GID/information-density primitives well-conditioned to ≤1 ULP — no catastrophic cancellation, no guard needed) · **B4** (Eq-23 cap differential oracle; cap-biting network = `logistic3_k8`, asserted to strictly bind — the network N1/P17 need) · **B5** (cross-formalism differential testing — all four invariants: `2026≤2023`, 3.0/4.0-reducibility refutation, `b3aaa3e5` 4.0 non-regression oracle, AC/IIT sign agreement) · **CES-completeness search** (brute force settles P11.95c case (b)'s open theory: the CES is **not** a complete substrate invariant in general — non-isomorphic substrates share an identical CES, down to repertoires/relations, whenever a unit is left unconstrained — but it **is** complete on *complexes*: 0 counterexamples among irreducible systems, n=2 exhaustive + n=3 sampled; `test/integration/test_ces_completeness.py`) · **P11.95c (a)+(c)** (substrate canonicalization — `pyphi/automorphism.py` with `substrate_automorphisms` / `substrate_canonical_form` / `are_substrates_isomorphic` / `canonical_state`, by exact behavior-aware node-permutation enumeration; **pynauty rejected** — it canonicalizes wiring not behavior, and Φ caps n below where n! matters. The per-direction φ asymmetry was confirmed a **reducible-system-only tie-break determinism issue** (the per-pair φ table is permutation-symmetric; the system MIP is non-unique when φ_s=0), fixed via a `canonical_state` Determinism tie-break in the IIT 4.0 SIA fallback; un-xfails `test_sia_per_direction_phi_multiset_symmetric`; `test/test_automorphism.py`) · **B16** (first-class `Complex` marker type: `complexes()` → `tuple[Complex, ...]`, null-object `maximal_complex()`, exclusion postulate as `validate.non_overlapping()`; advances ship-criterion #1) · **B20** (substrate ↔ networkx bridge; inferred-connectivity default) · **P14b env-gen** (matching environment generators: segment/point/noise + superpose/mixture/sample in `pyphi/matching/environment.py`; world distribution comes from stimulus generators, not a substrate stationary distribution) · **B13 scheme-constraint** (eager reject of IIT 3.0 + a non-bipartition system partition scheme, mirroring the reactive `sia_partitions` raise; EMD-precision deferral closed — POT is numerically clean; IIT 4.0 left open) · **P14b analytical-projection** (closed-form differentiation D via inclusion-exclusion over unique triggered structures; the only path to D when relations are analytical; perception-max D_p stays research) · **P14d A-5** (higher-degree relation viz: degree-≥4 faces drawn as star expansions, projection carries all degrees, `higher_faces`/`degrees=` controls, `view="spectrum"` φ-by-degree panel) · **Hypergraph view** (renamed from `simplicial_complex`; relation-as-star default + opacity fade + mechanism-anchored coincident-purview placement + rich relation/distinction hovers; `layout="embedding"` MDS/PCA composition embedding; + embedding block-weight/planar knobs and shared scatter/embedding PCA helper) · **P11.8 Tier 2** (benchmark suite rebuilt on the golden harness + nightly ASV wall-time trend; deterministic cProfile call-count gate blocks PRs; `test/integration/test_perf_counters.py`, `.github/workflows/benchmark.yml`) · **ASV nightly build fix** (the managed-env ASV path the nightly workflow depends on never ran; fixed three breakages — hatch-vcs `fallback_version` for git-less builds, `python -m build` so the wheel lands in asv's cache, and the `emd` extra in the managed install) · **Parallel engine unification** (two coexisting map-reduce implementations collapsed to one: `MapReduce` class → `pyphi.parallel.map_reduce()` function over the Scheduler Protocol, all 10 call-sites migrated, vestigial execution tree deleted, helpers deduped, backend selection now honored; ~900 lines removed, result-preserving; prerequisite for B18) · **B18 adaptive chunking** (activated `size_func`: pure `cost_balanced_partition` LPT packing + a `num_workers` chunk-count floor — de-risked at 2.8–3× with no overhead penalty; cheap cost proxies wired at relations/purview/concept; result-preserving) · **logging rework** (silent by default — a `NullHandler` on the `pyphi` logger, the root logger untouched, no auto-written `pyphi.log`; `pyphi.enable_logging(level, file)` is the opt-in; the three `log_*` config options + the `configure_logging` callback removed) · **substrate_modeler mechanism library** (ported Bjørn Juel's `substrate_modeler` into `pyphi.substrate_generator`: 16 unit mechanisms in `mechanisms.py` incl. the state-dependent endorsement `resonator`, the 6 composite-combination strategies + `composite()` in `mechanism_combinations.py`, and a `create_substrate(node_params)` factory; the stateless self-loop mapping reproduces the original's `dynamic_tpm` byte-for-byte — golden fixtures from the original, the matching-paper perceptual substrate built natively and run through the P14b `PerceptualSystem`; lets the matching repo drop `substrate_modeler`. Excluded: `stabilized_sigmoid` reproduces the documented intent not the original's swapped-axis bug; the original `mismatch_pattern_detector` is broken and asserted to intent instead)

### ⬜🟡 Remaining 2.0 work — *the schedule lives in [Remaining 2.0 Work](#remaining-20-work)*

| Item | Status | Wave | One-line |
|---|---|---|---|
| Single-formalism presentation | ✅ landed | 1 | Docs, README, MCP content and skills present the default as IIT (no "IIT 4.0 (2026)" labels; φₛ defined directly as min(φ_c, φ_e, ii)); earlier versions documented once in `docs/howto/earlier-versions.md`; card shows the Formalism row only for non-default results; MCP `formalism="iit3"` example fixed |
| IIT Expert distribution | ✅ landed | 1 | IIT Expert (learniit.org) published as one plugin for Claude Code, Codex and Cursor (`wmayner/iit-expert-plugin`, bundling the `iit-expert` skill and the `mcp.learniit.org` connector); `pyphi-mcp install` retires the `iit` skill (removing copies earlier versions wrote) and offers the plugin through each agent's own plugin commands (`--iit-expert`/`--no-iit-expert`); Cursor no longer gets a duplicate `pyphi` skill; new docs hub `docs/howto/ai-assistants.md` with a landing-page card each for IIT Expert and the MCP server. Spec/plan: `docs/superpowers/{specs,plans}/2026-09-26-iit-expert-distribution*` |
| Version-only override mislabels results | ✅ landed | 1 | `config.override({"iit.version": "IIT_4_0_2023"})` (or `version:` alone in `pyphi_config.yml`) was accepted and kept the default measure, labeling a current-definition value (Fig 1A aB φₛ 0.0405, not 0.1719) as 2023. The `system_measure_matches_requirement` constraint (`pyphi/conf/constraints.py`) now requires the system measure to apply the Eq. 23 requirement exactly when the version is defined by it, in both directions |
| Cache-key hash collisions | ✅ landed | 1 | `FrozenMap.__hash__` hashed the key set and the value set separately, so the 2ⁿ mechanism-state conditions the repertoire cache keys on collapsed onto three hashes and every cache operation became a linear scan under `Mapping.__eq__` — quadratic in cache size, present since the class was written (`f99b6694`, 2022-11-21) and reported externally. No value changes; a 10-unit specified-state search drops 55 s → 0.2 s, a 16-unit one from days to ~1 min. The same call also cached one full repertoire per system state and read each back once (order 4ⁿ cells, zero hits), exhausting memory at 16 units; the full-state sweeps now run under a `transient_repertoires` scope (14 units: 1.2 GiB → 0.14 GiB). **Harness gaps closed in the same change**, since neither the φ goldens nor the call-count gate could see a cost-per-operation regression: a declared registry of every cache-key type with a hash-separation assertion plus an instrumented discovery test that fails on an undeclared key type; collision-handling frames added to the perf-counter pins (they move on the 3-unit fixtures, so this was catchable in 2022); a `specified_state` perf grain with 10- and 12-unit ring fixtures, above the golden zoo's 4-unit ceiling; a cache-occupancy assertion at 10 units (11 vs 11,264 admissions); and a `specified_state_evaluations` axis in `estimate_analysis` + the MCP guard, which previously reported the state space as a decorative weight and charged the search to no axis (limit pinned to the kernel's own sweep bound, 16 binary units). Calibrating that limit exposed a third defect: the sweep bound was checked only on the effect direction while `Direction.both()` walks cause first, so an oversized system spent its entire cause sweep before anything refused — both directions now share one check. Every guard verified to fail against the reverted bug. **Follow-up 2026-08-15:** profiling the suite's slowest test (`fig6d_specialized`, 290 s, ~40% of the slow lane) found the mechanism-MIP sweep materializing its partitions before evaluating any — 31.9 M built to evaluate 4.4 M, the short-circuit stopping evaluation but not construction. Consuming them lazily (completeness now read from the memoized `partition_sweep_count` instead of `len()`) is 20% faster and 61% lighter, bit-identical across every φ, MIP, specified state and partition margin. The same profile reversed the `FrozenMap.__eq__` rejection: that workload calls `Mapping.__eq__` 18.4 M times rather than 76, where the fast path is worth 8.6%; `frozen_map.py:__eq__` joins the pinned frames so the sentinel still fails the same 11 tests against the reverted hash. |
...
| N1 paper-reproduction suite | ✅ landed | 1 | Acceptance suite (`test/integration/test_paper_reproduction.py`) reproduces every published worked example with paper-sourced pins: **IIT 4.0 (2023) Figs 1/2/4, all of Fig 6 (A–E), all of Fig 7 (A/B/C)**; IIT 3.0 (2014) Fig 12; AC 2019 Fig 6; Gómez 2020 Fig 3A. **2026-09 (2.0.0 release pass):** Mayner et al. 2026 Figs 2/3/4, Marshall et al. 2023 Figs 1/2/3, AC 2019 Figs 7–16 (`test_paper_reproduction_ac.py`, incl. the first multi-valued AC reproduction), Barbosa 2020 Figs 2–4 (with documented deviations where PyPhi's thresholds differ from the papers' quoted values). The previously-blocking **Fig 6 A/B/D/E + Fig 7 weight matrices (graphical-only in the paper) were supplied by the authors** and ship as `examples.iit4_2023_fig6{a..e}_substrate` / `iit4_2023_fig7_substrate` / `iit4_2023_fig7_inactivated_substrate` (+ `*_system`); 2.0 reproduces the **published** φ_s/Φ/distinction-relation counts to 2-dp (6A bottleneck, 6B modular, 6C copy-ring, 6D specialized Φ=11452 via analytical relations, 6E exclusion; Fig 7 A/B with 13740/13111 relations, Φ=22.26/18.55; **Fig 7C** E-*inactivated* φ_s=0.11, 14 distinctions, 363 relations, Φ=3.35). **Fig 7C resolution:** "inactivation" is a *lesion* — E's state frozen into the dynamics (`fig7_substrate().inactivate({"E": 0})`, the `Substrate.inactivate` lesion), a **distinct** operation from a background-conditioned subsystem; built that way 2.0 reproduces the panel exactly. The discrepancy localized to mechanism {A} (the unit most coupled to E, weight 0.8) and was verified **byte-identical to the pre-refactor `b3aaa3e5`** (no regression). For **Fig 6**, the authors' *working notebooks* carry pre-final φ magnitudes differing from their own published figures (a scheme sweep over all 8 system schemes reproduces none); 2.0 matches the published *figures* — so only the notebooks' weights + structure are reused. |
...
| N8 provenance stamp | ✅ landed | 2 | `Provenance` record (`pyphi/provenance.py`: pyphi version, git sha + dirty flag, ISO-UTC timestamp, wall-time, optional seed, Python/numpy/scipy versions, platform) auto-stamped as a sibling `provenance` field alongside the P10 `config` snapshot on the four top-level result types (IIT4 SIA, IIT3 SIA, CES, AcSIA). Wall-time filled at the IIT `sia`/`ces` compute entry points (`AcSIA` carries provenance without wall-time); equality stays φ-based so provenance never pollutes comparisons, diffs, or goldens; `pyphi.serialize` round-trips it. Renders at a new top display tier `PROVENANCE` (`repr_verbosity=4`). Users can re-stamp their own `note`/`seed` via `result.with_provenance(...)`. |
...
| P14d A-5 | ✅ landed | 2 | Degree-≥4 relation faces are drawn as star expansions (hub at the face centroid, sized/colored by φ, spokes to each endpoint) in the hypergraph view (then named `simplicial_complex`) — previously dropped. Projection carries all face degrees; `higher_faces` show element (default-on) + `degrees=` knob; new `view="spectrum"` φ-by-degree panel (count + Σφ). Convex-hull blobs deferred; skeleton expansion rejected. |
| Hypergraph view overhaul | ✅ landed | 2 | Post-A-5 refinement of the relational 3-D view, **renamed `view="simplicial_complex"` → `view="hypergraph"`** (it draws a hypergraph — MICE vertices, relations as hyperedges of any arity — not a downward-closed simplicial complex). Every relation is a star expansion by default; `star_min_degree` restores degree-2 lines / degree-3 triangles. Faces fade by opacity on a fixed hue (low-φ recede); coincident purviews separated by leaning each toward its mechanism (deterministic, collision-free; `endpoint_placement`); rich hovers for relations and distinctions; one shared `relation φ` colorbar. New `layout="embedding"` positions each MICE by a deterministic composition embedding — `embedding_method` `"mds"` (default, classical MDS of a purview-overlap distance) or `"pca"` (`render/embedding.py`). `SimplicialComplexGeometry`→`HypergraphGeometry`; theme knobs for hue/spokes/hub-size. Follow-ups landed (2026-06-22): embedding block weights exposed as `HypergraphGeometry` knobs (`embed_purview_weight`/`embed_mechanism_weight`/`embed_direction_weight`); flat 2-D embedding toggle (`embed_planar`); the sign-fixed PCA SVD and coincident-spread are now shared between the scatter and embedding renderers (`render/common.py` `spread_coincident` + `render/embedding.py` `pca_embed`, byte-identical scatter output). Deferred: convex-hull blobs. |
...
| P6b | ✅ landed | 2 | graphillion **removed** (not ported to OxiDD). A pre-design spike showed the ZDD is not load-bearing: the relations candidate family equals `combinations_with_nonempty_intersection` over distinction purview-unions (byte-identical on real networks; pure-Python is faster in the concrete-relations regime, and the >10⁶ regime uses `AnalyticalRelations` rather than concrete enumeration). Concrete relations enumeration is now a lazy pure-Python DFS (`pyphi/combinatorics.py`, prunes on empty running intersection); the `powerset_family`/`union_powerset_family` ZDD wrappers are gone. Closes the last no-GIL gap (no C extension on the relations path) and drops the macOS libomp source-build. OxiDD rejected: its Python ZBDD bindings lack enumeration/`set_size`/list-construction, so a port would swap one C-ext dep for a younger one to preserve a structure the spike proved unnecessary. |
...
| B13 config validator | ✅ landed | 2 | Eager rejection of cross-field config conflicts (`pyphi/conf/constraints.py`): measure↔version combos, and now `IIT_3_0` + a `system_partition_scheme` outside `{DIRECTED_BIPARTITION, DIRECTED_BIPARTITION_CUT_ONE}` (mirrors the reactive `sia_partitions` raise eagerly via a shared `compatible_system_partition_schemes` source of truth on the formalisms). Two-field error + fix; `validate_config` opt-out; failed apply restores state. Confirmed `2023+II` is *valid* (cap follows the measure, not the version) — so no `II⟹2026` rule. **Both deferrals closed by confirmation experiment (seed 20260618):** EMD under POT is numerically clean (noise floor machine-epsilon; IIT 3.0 phi stable across precision 6–13, identical MIP) → no precision constraint; IIT 4.0 computes a well-defined per-scheme phi for any system scheme → left open (`compatible_system_partition_schemes = None`). |
...
| B20 substrate graph bridge | ✅ landed | 2 | `Substrate.to_networkx()`/`from_networkx()`, `System.to_networkx()` (state + membership node attrs), GraphML/adjacency export, and IIT-relevant topology helpers (`pyphi/graph.py`). Defaults to the **TPM-inferred** causal connectivity (B19 `infer_cm`) so over-specified-`cm` phantom edges are dropped; `connectivity="declared"` uses `cm` verbatim. `from_networkx` inherits B19's under-specified-topology rejection (the inverse footgun — silently marginalizing a real edge — is a `ConfigurationError` by default). networkx stays optional (deferred import, `visualize` extra; never loaded at `import pyphi`). Excludes weights and BN/CPD semantics (→ N11). |
...
| P17 | ✅ landed (Waves A + C; Wave B deferred) | 6 | Cross-formalism perf characterization (post-freeze, internal-only). Wave A (local) complete: 2026-cap cost closed (cap is free); the IIT 4.0 speedup characterized + the earlier "19–43×" corrected as a scope artifact (real gain ~18–20× per SIA partition, ~2× CES); hot-path config sweep clean (no behavior mismatch; both pre-2.0 config bugs — `PARALLEL=False` still spawning, IIT 3.0 + GID raw crash — confirmed fixed); seeded 6–7 node Ising generator + n=6 sizing (full-search ~24 s/trial profiled; partition count grows ~7.8×/node, so n=7 ≈ minutes). Wave C (optimization) closed — no bounded win, the ~18× gain is already realized by the `core/repertoire_algebra.py` rewrite (the remaining lever is the larger N5 Rust-kernel item). Wave B (6–7 node server matrix) reclassified ⛔ deferred 2026-07-12: measurement-only, no code or φ implications, blocked on lab SSH — see the deferred table. `benchmarks/iit_3_vs_4/README.md` |
...
| Documentation overhaul (2.0 / IIT 4.0) | ✅ landed | 6 | Toolchain & skeleton landed (pydata-sphinx-theme + myst-nb, six-section layout, build-time API reference, docs CI, RTD build fixed). Final-state docstring sweep landed: every `pyphi/**` docstring rewritten to NumPy-style final-state voice (substitution markup → Unicode symbols, Google → NumPy sections uniformly, `napoleon_google_docstring = False` enforcing it), with paper-verified literature citations; surfaced code-level bugs recorded for follow-up in `docstring-sweep-findings.md`. Theory narrative landed: an eight-page IIT 4.0 theory section under `docs/theory/` (postulates → φ-structure, the paper-to-code map, formalism versions, conditional independence, IIT 3.0), computation-driven and grounded in one build-time-executed worked example, with every equation paper-verified. Migration guide landed: `docs/migration/migration-2.0.md`, a topic-organized guide with [1.x]/[4.0-branch]/[both] audience tags (rename table + per-topic before/after for entry point, formalism selection, config, serialization, changed defaults), every 2.0 snippet verified against the code. Tutorials & how-tos landed: a getting-started walkthrough, five tutorial pages (cause-effect structure, a complete XOR worked example, macro, actual causation, and the IIT 4.0 paper-supplement notebook faithfully updated to the 2.0 API in place at its published path `docs/examples/IIT_4.0_demo.ipynb`), and seven how-to guides — all executable MyST verified at build time. The last `.rst` example/config/tie-breaking pages were retired, `conventions.rst` de-substituted, and the `rst_prolog` substitution block deleted, so every page now uses inline math/Unicode. All five sub-projects have landed; the documentation overhaul is complete. Subsumed the scattered P15 docs bullets + ship-criterion #5. See [Documentation overhaul](#documentation-overhaul-20--iit-40). **2026-default rework (2026-07-12):** teaching moved onto the promoted `iit4_2023_fig1a` example (nonzero under the shipping default) — getting-started and the theory pipeline pages run on it, and one worked tutorial follows the paper's Figs 1→2→4 (replacing the XOR worked example and the cause-effect-structure tutorial); determinism ⇒ φₛ = 0 taught explicitly (`docs/theory/intrinsic-information.md`, citation-verified against the 2026 paper); formalism-versions/configure/migration/what's-new/index state the 2026 default; how-tos example-swapped or pin-annotated; demo notebook pinned to 2023; MCP walkthrough + tool docstring + bundled theory reference corrected. Spec/plan: `docs/superpowers/{specs,plans}/2026-07-12-docs-overhaul-2026-default*`. |
...
| N7 `pyphi.analyze()` | ✅ landed | 6 | One high-level `analyze(substrate, state, formalism=…)` entry point. Pulled into 2.0 (no blocker). |
| N9 labeled-export audit | ✅ landed | 6 | Audit and close the unified `to_pandas`/xarray export story — largely delivered by P14d + B21 (`models/pandas.py` base + TPM/diff/explanation coverage); confirm full result-type coverage and fill gaps. Pulled into 2.0. Extended `to_pandas()` to every `Displayable` result type (SIA 3.0/4.0, CES, relations, AC family, `Complex`/`ExcludedCandidate`, `System`/`Substrate`/`Node`/TPMs, partitions/cuts, `UnitState`) with a coverage-invariant test; xarray confirmed TPM-only by design. |
...
| Selection-margin reporting | ✅ landed | 7 | Margins at every IIT 4.0 selection. System level: `partition_margin` (normalized-φ gap to the best competing system partition; `None` when the sweep short-circuited — no false margins over truncated sweeps), per-direction specified-state margins, `tied_selections` naming the effectively tied selections, `effectively_tied`. Mechanism level: `RIA.partition_margin`, `MICE.purview_margin`, `state_margin`, and `effectively_tied` on RIA/MICE/`Distinction`. Exclusion level (the theory's third selection): `Complex.exclusion_margin` (φₛ gap to the best overlapping rival the complex beat — beaten-rivals semantics; higher-φₛ shadows carried by the recursive cascade never enter) + `Complex.effectively_tied`, derived from exclusion records at both doors (micro `substrate.complexes` and macro grain search); `exclusion_records` identity fix so same-footprint rival grains are recorded. `shortcircuit_sia=False` now also disables the sweep-level early stop (system + mechanism), making margins exact everywhere with unchanged φ values. Surfaced via `explain()`, FULL-verbosity cards, `to_pandas`, serialization, relabel; consumed by `pyphi.sweep` rows (near-tied cell flagging) and `phi_posterior`'s margin-gated screening. IIT 3.0/AC deferred (different selection structure). Docs: tie-breaking how-to §Selection margins, theory §Selection margins, `docs/tutorials/recursive-exclusion.md`. Specs: `2026-07-07-substrate-parameter-landscapes.md` §7, `2026-07-09-grain-selection-margin-design.md` |
| Grain-search cost pre-flight | ✅ landed | — | `SearchBounds.estimate(substrate, limit=…)` → `SearchEstimate`: an analytic pre-flight counting what a `pyphi.macro.complexes` sweep would evaluate — no TPM construction, no φₛ. A counting walk in `pyphi/macro/estimate.py` mirrors `_derive_units`/`complexes` control flow and drives the search's real enumerators with multiplicity-carrying stand-in units; level-1 judgment counts exact, deeper levels + assembly sweep exact worst cases (all-pass), equality pins where the worst case is achieved (min fixture 8/10 = records). Reports per-level judgments/pools, distinct-system and assembly bounds, m-bucket weights with memoized live partition counts (`partition_sweeps_upper_bound`; `partitions_capped` above m=6), construction keys, `is_exact`/`truncated`; requires IIT 4.0 as the drivers do. N18 in miniature; grain-exposure item 2. Spec: `2026-07-10-grain-cost-preflight-design.md` |
| `analyze()` grain axis | ✅ landed | — | The grain search is reachable from the front door: `analyze(substrate, state, grains=True)` (or `grains=SearchBounds(...)`) dispatches to `pyphi.macro.complexes` under the same formalism handling as the single-system path and returns its `ComplexesResult`; `state` doubles as the micro history when the bounds' maximum micro grain exceeds 1. `grains` is mutually exclusive with `subset`/`compute` (`grains=False` and non-bounds values raise); `parallel_kwargs` forwards to the driver. Front-door pointers added to the package docstring and the macro tutorial. Grain-exposure item 3. Spec: `2026-07-10-analyze-grain-axis-design.md` |
...
| Formalism-default decoupling | ✅ landed | 7 | The test suite no longer silently rides the ambient default formalism. Every φ-asserting test pins its formalism explicitly, sourced from `pyphi.conf.presets` (a whole `IITConfig` as a unit) rather than a hand-listed subset of `iit.*` fields. Root fix: `test/golden/zoo.py`'s `IIT_4_2023_CONFIG` set `iit.version` + the mechanism measure but **omitted `system_phi_measure`**, silently inheriting it from the ambient default — a leaky pin that looked complete; now `{**presets.iit4_2023, …}`, mirroring `IIT_3_CONFIG`. ~39 previously-unpinned numeric tests pinned at the narrowest correct scope (module autouse / per-class / per-test / **inside module-scoped φ fixtures**, since a function-autouse pin does not wrap module-fixture setup); one intentionally-**unpinned** canonical assertion (`test_default_formalism_is_iit4_2023`) is the single flip signal. Zero φ-value changes, no goldens regenerated; measured flip blast radius is exactly 3 default-dependent tests. Convention + flip recipe documented in CLAUDE.md §"Formalism pinning". Emerged from the ii-gated-grain-scheduling brainstorm; **prerequisite for making 2026 the default**. Spec/plan: `docs/superpowers/{specs,plans}/2026-07-11-formalism-pinning-decoupling*`. |
| Default formalism → IIT 4.0 (2026) | ✅ landed | 7 | The library default formalism is now **IIT 4.0 (2026)**: `pyphi/conf/formalism.py` defaults to `version="IIT_4_0_2026"` + `system_phi_measure="INTRINSIC_INFORMATION"` (mechanism/specification stay GID), applying the Eq. 23 intrinsic-information cap `φ_s = min{φ_c, φ_e, ii(s)}` by default. Enabled by the formalism-default decoupling (row above): the pinned suite is inert to the flip, so it touched only `conf/formalism.py` + the handful of tests that *assert* the default (canonical assertion renamed `test_default_formalism_is_iit4_2026`; facade/snapshot/defaults assertions; `test_2023_omitted_metric_uses_default` now reads the capped default). **Root fix surfaced by the flip:** the `presets` were *diffs from the dataclass default*, so `presets.iit4_2023`/`iit3` silently inherited the new cap measure — made **absolute** (each preset now sets its measures explicitly, independent of the default), which also re-secured `IIT_4_CONFIG`/golden `IIT_4_2023_CONFIG` and the B5 pre-refactor byte-match oracle helper. No golden regenerated. Select `formalism="IIT_4_0_2023"` for uncapped IIT 4.0, `"IIT_3_0"` for IIT 3.0. Changelog: `default-formalism-2026.change.md`; migration guide + config reference updated. |
...
| Generative-relations compression (serialize "Lever 2") | Post-2.0 optimization **research**, not a blocker. Concrete relations are deterministically derivable from the distinctions (P6b: the candidate family is `combinations_with_nonempty_intersection` over distinction purview-unions), so a CES could persist its distinctions plus a generation flag and **regenerate** relations on load, instead of storing them at all. The serialize rewrite's normalization (distinctions stored once, relations referencing them by index) + msgpack already removed the dominant bloat (rule154 1.31 MB → 43 KB), so the remaining payoff is small and it carries **persist-vs-recompute semantics risk** (regeneration must be byte-identical; `AnalyticalRelations` vs `ConcreteRelations`; behavior if `config` differs on reload). Gate: re-measure new-format file sizes on real large structures; open only if they are still large. |
| AC default-flip | `noise_background` default — breaking change + unsettled AC background-condition semantics |
| P17 Wave B | 6–7 node cross-temporal benchmark matrix — measurement-only (no code, API, or φ implications; the informative findings all came out of Waves A/C), blocked on lab-server SSH access. Run opportunistically when access exists; does not gate the 2.0 ship. |
...
- **k-ary EMD — landed (2026-06-14).** EMD was the last IIT-3.0 measure still binary-only (`measures/distribution.py` registered it `supports_alphabet=_binary_only`, with `2^N`-Hamming kernels and the `# TODO extend to nonbinary nodes`). Generalized both directions to arbitrary/heterogeneous alphabets: the cause-direction ground metric (`_ground_metric`/`_kary_hamming_matrix`) now counts differing node states over the substrate's actual product state space (states enumerated via `all_states` in `flatten`'s little-endian order), and the effect-direction analytic shortcut (`effect_emd`) generalizes to per-node total variation between k-ary marginals. The binary path is byte-identical (delegates to the cached `_hamming_matrix`; full golden suite passes unchanged). EMD is now `_any_alphabet` and computes a system φ as the IIT-3.0 mechanism measure on the multi-valued p53 net (φ=0.5 under EMD vs 0.44 under the paper's AID). Validated by `test/measures/test_kary_emd.py` — known hand-computed values, metric properties, the analytic effect-EMD matching the full Wasserstein distance to machine epsilon on random k-ary product distributions, and a `multivalued_k3_tiny_iit3_emd` golden. Removed the now-dead `_binary_only` predicate and updated the alphabet-support tests that had enforced the limitation.
- **CES-completeness search — landed (2026-06-13, `test/integration/test_ces_completeness.py`).** Brute force on small binary substrates settled the open theory question gating P11.95c case (b). **Finding:** the CES is **not** a complete invariant of the substrate *in general* — non-isomorphic substrates can share an identical CES (down to repertoires and relations, under node relabeling) whenever a unit is left unconstrained by the structure (a causally inert unit, or a unit specifying no distinction at the evaluated state); concrete n=2 counterexamples are pinned. **But** restricted to *irreducible* systems — the complexes a CES is properly the structure of — **no counterexample is found** (n=2 exhaustive: 132 complexes, all distinct CES fingerprints; n=3 sampled: 250 complexes, 250 distinct). So case (b) is formally non-empty (the "open theory" blocker is removed), yet its only witnesses are reducible (Φ_s = 0) systems; on complexes, substrate canonicalization (a)+(c) suffices and the opt-in `intrinsic_equivalence` API is low-value. The fingerprint matches the roadmap's case-(b) CES-as-labeled-structure definition and uses specified-state *tie sets* so tie-resolution labelling cannot manufacture spurious differences.
- **N1 — paper-reproduction acceptance suite (CI gate) — *landed (2026-06-16, `test/integration/test_paper_reproduction.py`)*.** Systematically reproduce every published worked example (IIT 4.0 Figs 1/2/4/6/7, Marshall 2024, AC 2019 Fig. 11, Gómez p53) with pinned expected values, wired into CI (the fast reproductions gate merges in the Tests workflow; the slow-marked heavy panels run nightly with `--slow` via the `slow-tests` workflow). Subsumes the p53-Mdm2 golden above and *yields the network that exercises the 2026 ii-cap with non-zero φ* that P17 (Wave 6) needs. *(Promoted from the wishlist.)* **Landed so far — IIT 4.0 (2023) Figs 1, 2, 4 & 6C.** The Fig 1A logistic substrate (3 units; sigmoid activation Eq 60, k=4; weights read from the causal-model diagram: A↔B=+0.7, A→C=+0.2, C→B=−0.8, A/B self=−0.2, C self=+0.2) is reconstructed and its Fig 1E system-φ values reproduce the paper to two decimals — `φ_s(a)=0.04`, `φ_s(aB)=0.17`, `φ_s(aBC)=0.13`, plus aB's cause/effect split `φ_c=0.24`/`φ_e=0.17` (Fig 1D) — five published values matching to 2 dp, with aB confirmed a complex (maximal φ_s among overlapping systems). The matching values double as a checksum on the weight reading. **Cross-validated against paper-era PyPhi 1.2.0 (commit `75d0c411`):** both versions agree the single unit {C} has the highest φ_s here (~0.21–0.29), so aB is *a* complex, not the global max (the paper's exact claim); and current PyPhi's `DIRECTED_SET_PARTITION` is the paper-faithful scheme — it reproduces `φ_s(a)=0.04` where the old `SET_UNI/BI` scheme gives 0.068. **Fig 2** (distinctions of aB) and **Fig 4** (relations) extend the same complex: the three distinctions reproduce `φ_d(a)=0.33`, `φ_d(B)=0.32`, `φ_d(aB)=0.07` with their cause/effect purviews, and the relation `r({a,aB})` reproduces 9 faces and `φ_r=0.0357` (= `φ_d(aB)/2`; the paper's 0.035 is the same quantity from the rounded `φ_d(aB)=0.07`). The Fig 2/4 φ_d/φ_r values live only in the figure images and were read out by rendering the PDF figure graphics at high resolution. **Fig 6C** (one of five 6-unit nets) is the only Fig 6 panel whose weights are given *exactly in the text* ("a directed cycle… unidirectionally connected with weight w = 1.0 and k = 4. Each unit copies the state of the unit before it"): the copy-ring reproduces `φ_s = 1.7467` (paper's 1.74 is a 2-dp truncation) and `Φ = 7.65`, where Σφ_r is computed with `pyphi.relations.AnalyticalRelations(...).sum_phi()` (Albantakis 2023 S3) — sidestepping concrete-relation enumeration (the recipe for the heavy panels, e.g. 6D's ~1.5M relations / Φ≈11452). The 6-unit `sia()` takes ~27 s, so the test is `@pytest.mark.slow`. *(Method note: the 4.0 paper's worked examples are logistic networks with weights given only graphically — the existing `fig*` examples are from the 2014 3.0 paper, not 4.0 — so each figure must be reconstructed from its diagram and validated by its published φ; networks are built with `pyphi.substrate_generator.ising.probability` at `temperature=1/k`, equivalent to the Fig 1 sigmoid; this is now a working recipe.)* **Fig 6 A/B/D/E + Fig 7 — landed (2026-06-16, unblocked).** The authors supplied the exact weight matrices (previously graphical-only); they now ship as `pyphi.examples.iit4_2023_fig6{a..e}_substrate` / `iit4_2023_fig7_substrate` (+ `*_system`), built with `substrate_generator.ising.probability` at `temperature=1/k`, k=4 — the Fig 6C recipe. Current 2.0 reproduces the **published** figure values to 2-dp display precision: 6A bottleneck (complex Ab φ_s=0.05/Φ=0.48; monad c 0.008/0.02), 6B modular (Ab 0.53/2.74; cd=ef 0.45/3.18), 6D specialized (full-system φ_s=1.05, 27 distinctions, Φ=11452 via `AnalyticalRelations(...).sum_phi()`), 6E exclusion (Abef 0.27/3.24 is the first complex; the full 6-unit system φ_s=0.15 is excluded), and Fig 7 A/B (full system in states ABcdE/ABcde: φ_s=1.1/1.31, 23 distinctions, 13740/13111 relations, Φ=22.26/18.55). **Confirmation experiment (don't-defer):** the authors' working notebooks carry *stale* φ magnitudes from a pre-2.0 system-partition scheme (the removed `SET_UNI/BI`) — a sweep over all 8 system schemes, both φ_d normalizations, and all 3 mechanism schemes reproduces *none* of them, while the distinction/relation **counts match exactly**. So only the weights + structure are reused; 2.0 matches the *paper* (as for Figs 1/2/4/6C; `DIRECTED_SET_PARTITION` is the sole scheme reproducing the Fig 7 A/B published φ_s). **Fig 7C (E "inactivated") — reproduced via the lesion construction.** Inactivation is *not* a background-conditioned subsystem: it freezes E's state into the dynamics (E loses its counterfactuals), realized as `fig7_substrate().factored_tpm.condition({4: 0})` → `iit4_2023_fig7_inactivated_substrate` (E's strong −0.8 input to A becomes a fixed bias). Built that way 2.0 reproduces the panel exactly (φ_s=0.11=`min(cause 0.61, effect 0.11)`, 14 distinctions, 363 relations, Φ=3.35). The investigation: the whole discrepancy localized to mechanism {A} (the unit most coupled to the inactivated E), the 4-unit *subsystem* treatment (`node_indices=ABCD`, E external) gives φ_s≈0.05 / Σφ_d=2.09, and the *inactivation* (E conditioned into the TPM) gives the published 2.90/363/3.35 — confirming the two are distinct IIT operations. Verified **byte-identical to the pre-refactor `b3aaa3e5`** for both the subsystem and the SIA effect direction (no 2.0 regression). The 6D full `sia()` + analytical Φ (~5 min) is `@pytest.mark.slow`; Fig 7 A/B are slow, Fig 7C is fast (4-unit complex). **IIT 3.0 (2014) Fig 12 — landed (2026-06-14).** The 2014 paper's *worked* example (Figs 4/6/8–12/14) is the 3-unit logic-gate net A=OR/B=AND/C=XOR (fully connected; `pyphi.examples.fig4_substrate`) in state (1,0,0) — note the roadmap's earlier "Fig 1" label was loose: the 2014 Fig 1 is the conceptual *Existence* axiom, not a Φ example, and `fig4_system` fixes the *different* state (1,0,1). Under the `iit3` preset PyPhi reproduces Fig 12's `Φ^MIP = 1.92` exactly (computes 23/12 = 1.9167) and its six-concept constellation with φ^Max {0.5, 0.33, 0.25, 0.25, 0.17, 0.17} (per-mechanism A=B=1/6, C=AB=1/4, BC=1/3, ABC=1/2; mechanism AC specifies nothing). Two independent published sources agree — the 2014 Fig 12 figure value and the Mayner et al. 2018 "Calculating φ" practical guide's `Φ = 1.917`. (The published value is figure-only in body text, as with the 4.0 figures, so it was read from the rendered Fig 12 and confirmed against both the computation and the 2018 guide.) **Actual Causation (2019) Fig 6 — landed (2026-06-14).** The canonical 2-unit OR-AND example (`pyphi.examples.actual_causation_substrate`); under the `iit3`/PMI preset PyPhi reproduces Fig 6's full causal account of the transition {OR,AND}=10→10 — four first-order links (OR & AND, each as cause and effect) at α = log₂(4/3) = 0.415 bits and one second-order joint *cause* link at α = log₂(9/8) = 0.170 bits (no irreducible joint effect). As with "Fig 1" above, the roadmap's "Fig 11" label was loose: 2019 Fig 11 is the 7-unit "voting" net (weights graphical-only, not in `pyphi.examples`); Fig 6 is the canonical example the library ships. **Gómez 2020 Fig 3A (multi-valued p53-Mdm2) — landed (2026-06-14):** `Φ(001)=0.44` over 3 mechanisms via the paper's `AID`/wedge-tripartition/`SUM_SMALL_PHI` config (see the p53-Mdm2 golden item above), the suite's first k>2 reproduction. **All N1 reproductions are now landed** (Fig 6 A/B/D/E + Fig 7, formerly blocked on the authors' definitions, landed 2026-06-16 — see above). **P11.95e fallout — resolved (2026-06-17).** The IIT 3.0 `rule110`/`grid3` `sia.phi` goldens depend on the `PURVIEW_SIZE` purview tie-break (the audit proved it load-bearing for system φ) and previously lacked an independent reference — only `basic`/`basic_tri` carried canonical values. They are now anchored to a genuine PyPhi 1.2.0 IIT 3.0 SIA (CPython 3.9; `scripts/gen_iit3_emd_oracle.py`), with the oracle's correctness controlled by reproducing the anchored `basic` value (2.3125, exact, with the matching cut and concept set) before it is trusted on the unanchored nets. Under the golden's exact config (EMD mechanism + EMD CES-distance, `PARTITION_TYPE='BI'`, precision 6) and PyPhi 1.x's default larger-purview tie-break (`PICK_SMALLEST_PURVIEW=False`), the oracle reproduces both goldens to 6 dp: `rule110`(1,0,1)=**1.100015**, `grid3`(1,0,0)=**0.020341**. The tie-break sensitivity is real and confirms the audit — resolving toward the *smaller* purview instead gives 1.033765 / 0.044527 — but 2.0's `purview_tie_resolution=['PHI','PURVIEW_SIZE']` (applied with `operation=max` in `resolve_ties.purviews`, i.e. prefer the larger purview) matches the 1.x default, so the goldens are correct, not locked to a tie-break artifact. References: `test/data/iit3-canonical/{rule110,grid3}_sia_phi_canonical.json`; enforced by `test/integration/test_golden_regression.py::test_iit3_emd_sia_phi_matches_pyphi_1x_reference`.
- **B5 — cross-formalism / pre-refactor differential testing (landed in full).** *Landed (2026-06-13, `test/integration/test_cross_formalism_invariants.py`):* the `φ_2026 ≤ φ_2023` Hypothesis invariant — which **caught a real Eq-23 cap MIP-selection bug** (the cap was applied per-partition inside the system-MIP search, shifting the MIP so 2026 φ could exceed 2023 φ; now fixed, MIP selected on uncapped φ and capped once) — plus the **refutation** of the hypothesized "3.0/4.0 agree on reducibility" invariant (they disagree on ~70% of random reachable small substrates; a reachable witness is pinned). *Landed (2026-06-14):* the 4.0-only non-regression oracle against the `b3aaa3e5` pre-refactor commit (`TestPreRefactorByteMatch`; reproducer `scripts/gen_iit4_2023_byte_match_oracle.py`, data `test/data/iit4_2023_byte_match_oracle.json`). A 48-state corpus (45 nonzero: 20 positive complexes and 25 negative "preventative cause" systems, plus 3 zero) is recomputed and matched under the **`DIRECTED_BIPARTITION`** scheme (the oracle's `DIRECTED_BI`, *preserved* across the refactor — **not** the era's default `SET_UNI/BI`, which was intentionally replaced by `DIRECTED_SET_PARTITION`). Two findings made a *literal* `float.hex()` match infeasible, neither a regression: **(1)** the pre-refactor `phi` was the **raw, un-clamped** integration, so it matches HEAD's `signed_phi`, while HEAD's clamped `phi` = `max(0, raw)` (the Eqs. 19-20 `|·|+` correction the refactor added); **(2)** sub-ULP floating-point reassociation (max abs diff **3.3e-16** over the corpus), so the test asserts machine-epsilon equivalence (`atol=1e-12`). Net: the 4.0 directed-bipartition hot path is confirmed numerically non-regressing across the 2.0 refactor. *Landed (2026-06-14):* the **AC/IIT sign-agreement invariant** (`TestAcIitSignAgreement`). The "unsettled" relation resolves cleanly at the primitive level: actual causation's α and IIT's integration are both `log2(p/q)` of the *same* repertoire value at the actual purview state — AC's α (PMI) reads `p = transition.probability(...)` / `q = partitioned_probability(...)`, and `transition.probability` delegates to the very `system.repertoire` IIT's GID uses, where `GID_at_state = selectivity_at_state · log2(forward/partitioned)` with the selectivity repertoire ≥ 0. So the structural identity `GID = selectivity·α` holds *exactly* (verified to 1e-9 across 324 mechanism/purview/partition combos on the OR-gate transition + a Hypothesis property over random small substrates), and the sign-agreement corollary follows: the two formalisms never disagree on the *direction* (excitatory vs "preventative") of a fixed mechanism/purview/partition. **B5 complete.** *(Note: a 3.0 byte-match is **not** viable from `b3aaa3e5` — its `compute.phi` is not a valid IIT 3.0 SIA, `basic` → 0 — so it would need a genuine PyPhi 1.x oracle.)*
- **B22 — raw-float φ-comparison audit.** ✅ **Landed.** The codebase-wide sweep for φ/Φ/α comparisons performed on raw Python floats (equality, ordering, deduplication, or selection decisions that must respect `config.numerics.precision`) found and fixed eight unsafe sites. Rather than wrap every quantity, tolerant comparison is consolidated at the decision sites: the scalar predicates in the new `pyphi.numerics` module (`eq`, `is_zero`, `is_positive`, `is_nonpositive`, `positive_mask`, `round_to_precision` — the `utils.eq`/`is_positive`/`is_nonpositive` helpers moved here) and `resolve_ties`' tolerant float-key clustering, which co-selects candidates tied up to precision regardless of iteration order. The `PyPhiFloat` wrapper is deleted; `DistanceResult` is now a plain-float metadata carrier (serialization schema `PhiSchema = float | DistanceResultSchema`; fixtures regenerated value-identically). Actual causation gained a system-level tie cascade (`resolve_ac_sia_tie`/`resolve_ac_nexus_tie`) with tie bookkeeping (`AcSystemIrreducibilityAnalysis.ties`), and the `INTRINSIC_DIFFERENTIATION` noise-surprisal filter now respects precision. An AST lint (`test/test_precision_lint.py`, with named waivers) gates recurrence. Spec: `docs/superpowers/specs/2026-07-10-precision-comparison-architecture-design.md`. Sibling to the Complex-unification work that surfaced it.
...
- **N8 — provenance stamp on results — landed (2026-06-18).** A frozen `Provenance` value type (`pyphi/provenance.py`) records pyphi version, git sha + dirty flag, an ISO-UTC timestamp, wall-time, an optional RNG seed, the Python/numpy/scipy versions, and platform (system/machine). It is auto-stamped at construction as a sibling `provenance` field alongside the P10 `config` snapshot on the four result types that carry one (the IIT 4.0 SIA, IIT 3.0 SIA, `CauseEffectStructure`, and `AcSIA`); the git lookup is cached once per process. Wall-time is filled at the public IIT `sia` / `ces` compute entry points via a frozen-replace helper, so a directly-constructed result (and an `AcSIA`) keeps `wall_time=None`. Provenance never pollutes value comparisons: equality stays φ-based, `diff()` ignores it, and the golden harness extracts named fields, so timestamps and host details never enter a fixture; `pyphi.serialize` round-trips it. It renders only at a new top display tier, `PROVENANCE` (`repr_verbosity=4`, above `FULL`), leaving the default repr unchanged. Users can record their own context on a computed result via `result.with_provenance(note=…, seed=…)` (a free-form `note` field plus the existing `seed`), which updates the frozen record in place and returns the result. Hostname was dropped (privacy); automatically threading seeds through stochastic paths and the N4 disk cache that will consume this stay out of scope. *(Promoted from the wishlist.)*
- **P14b tail — environment generation (landed 2026-06-18) + analytical projection.** *Env-gen — landed:* `pyphi/matching/environment.py` ships the manuscript's stimulus generators as pure functions producing a world distribution (`dict[tuple[int, ...], float]` over the sensory interface, exact over all `2^n`): `segment`/`point`/`noise`, composed via `superpose` (independent activation combined by elementwise OR — the paper's overlay-on-noise) and `mixture` (weighted choice), with a seeded `sample` (isolated `np.random.default_rng`) for inspection. All six are exported from `pyphi.matching`; `MatchingAnalysis` runs without a hand-supplied world distribution, and the paper's E1/E2/E1b/E3 reproduce to pinned per-stimulus probabilities (`test/matching/test_environment.py`). **Scope correction from reading the manuscript:** the world distribution comes from stimulus generators, *not* a substrate stationary distribution — so `stationary_distribution` / the Metropolis-Ising sampler are substrate-construction tools, out of scope here (deferred; would live in `substrate_generator`). Temporal/sequential stimulus models stay out too (matching samples i.i.d. from the world distribution). *Analytical projection — landed (2026-06-18):* `Differentiation.analytical_differentiation` computes the φ-maximized cross-structure differentiation D (Eq 16) in closed form, by inclusion-exclusion over the unique triggered structures — `Σ_{∅≠T} (-1)^(|T|+1) AnalyticalRelations(∩_{k∈T} D_k).sum_phi()`, `2^K-1` analytical relation-sum calls, K = unique structures. It reads only each structure's distinctions, never enumerates a concrete relation, and is cross-validated against the concrete `differentiation` (grid3 disjoint, single-structure, duplicate, and a partial-overlap sub-CES case that exercises the strict-subset cross term). It is also the *only* path to D when the structures carry `AnalyticalRelations`, which are deliberately non-iterable (the concrete `differentiation` raises trying to walk them) — so this is a functional gap, not merely an optimization. *Perception-maximized projection — open research:* D_p's per-component weight is φ_r · mean-of-triggering, and the mean factor breaks the pure-min algebra, so D_p stays concrete.
- **P14d — `to_pandas` consolidation + A-5 viz — landed.** The per-class `ToPandasMixin` is unified into one convention (`pyphi/models/pandas.py`): a thin `to_pandas()` delegates to a per-class `_to_pandas()`, with pure helpers (`record_to_series`, `records_to_frame`, `state_multiindex`, `distribution_rows`) owning all pandas construction over the existing `NodeLabels` utilities. Shapes are category-determined — scalar-record (RIA/MICE/`Distinction`)→`Series`; `Distinctions`→`DataFrame` indexed by labeled mechanism; `StateSpecification`/`SystemStateSpecification`→tidy long-format (`direction, kind, purview, state, probability`, the pair = `concat`); `TriggeredTPM`→wide labeled matrix rebuilt on the shared `state_multiindex` helper (output byte-identical, guarded by a test). Units render as labels (k-ary aware via `all_states`/`flatten`; integer fallback where state specs carry no `node_labels`); the `json_normalize` heuristic and `Series`-vs-`DataFrame` guessing are removed. `test/models/test_to_pandas.py`. *(Also fixed a latent pre-commit gap: pyright lacked `pandas`, so it resolved `import pandas` in `pyphi/models/` to the sibling `models/pandas.py`.)* A-5 landed (2026-06-18): degree-≥4 relation faces are drawn as star expansions (a hub at each face's centroid, sized/colored by φ, with spokes to its endpoints), the projection carries all face degrees (previously degree ≥4 was dropped), a default-on `higher_faces` show element and a `degrees=` knob control which degrees render, and a new `view="spectrum"` φ-by-degree panel summarizes relation count and Σφ per degree. Convex-hull blobs deferred; skeleton expansion rejected.
- **P11.95c (a)+(c) — substrate canonicalization — landed (2026-06-15).** `pyphi/automorphism.py` ships `substrate_automorphisms`, `substrate_canonical_form`, `are_substrates_isomorphic`, and `canonical_state`, computed by **exact behavior-aware node-permutation enumeration** (preserving connectivity, TPM, and alphabet sizes). **pynauty was rejected** during planning: it canonicalizes a *colored graph* by wiring and cannot see the TPM (so AND/XOR nodes with identical wiring would be wrongly identified), and its only advantage is on graphs far larger than n! enumeration handles — a regime Φ never reaches (Φ is O(2ⁿ), so n ≲ 8 where n! ≤ 40320). **The open principle is resolved:** the per-direction asymmetry is *not* a bug in φ but a **reducible-system-only tie-break determinism issue** — the per-`(cause,effect)`-pair φ table is provably permutation-symmetric, and when every pair ties at φ_s = 0 the system MIP is non-unique, so the state-tie cascade's enumeration-order fallback was label-dependent. Fixed with a `canonical_state` Determinism tie-break at the IIT 4.0 SIA fallback (fires only on the residual φ_s=0 tie, and only for full-substrate systems — proper-subsystem reducible ties keep enumeration order; irreducible-system values are unchanged, and no golden drifted). Un-xfails `test_sia_per_direction_phi_multiset_symmetric`; `test/test_automorphism.py`. Case (b) stays out — the Wave-1 CES-completeness search settled it: CES is complete on complexes, so (a)+(c) suffice for the regime that matters; the residual incompleteness is reducible-system-only.
- **P6b — graphillion removed (not ported to OxiDD) — landed (2026-06-18).** The plan was to reimplement the `setset` family algebra behind a `ZDDFamily` Protocol with OxiDD as default. A pre-design spike (`experiments/p6b_spike/`) changed the conclusion: the ZDD is **not load-bearing**. graphillion's only use was the *concrete* relations candidate generation (`relations.py::_combinations_with_nonempty_congruent_overlap` via `powerset_family`/`union_powerset_family`), which `all_relations` builds and then *fully enumerates* into a `MapReduce` — so the ZDD's symbolic compression is never exploited. The family it produces is exactly the set of distinction-combinations sharing a common purview unit, i.e. `combinations_with_nonempty_intersection` over each distinction's purview-union. **Spike findings (seed 20260618):** byte-identical candidate families on every real network (basic/xor/residue/grid3) and across a synthetic sweep up to 4.6M combinations; pure-Python is *faster* up to ~10⁵, ties to ~10⁶, and the ZDD pulls ~2.7× ahead only at the maximal dense 5-unit case (4.6M combos, 25s vs 68s) — an irrelevant regime, since there generation time is noise against millions of φ evaluations and the code uses `AnalyticalRelations` instead. **OxiDD rejected:** its Python ZBDD bindings expose the boolean algebra but lack enumeration, `set_size(k)`, and list-construction — the three operations this code needs — so a port would swap one C-extension dependency for a younger one *and* write nontrivial ZDD code, to preserve a structure the spike proved unnecessary. **Landed:** a lazy depth-first `combinations_with_nonempty_intersection` (prunes a subtree the instant the running intersection empties — sound because intersection is monotone; streaming, O(depth) memory) with `_by_order` reimplemented as a thin grouping over it; relations rewired to it; the graphillion wrappers, the `Graphillion>=1.5` dependency + `[tool.uv.sources]` git override, and the graphillion no-GIL deferral test all removed. The property oracle also caught a latent bug in the old enumerator (it ignored `max_size < 2`); the DFS fixes it (no real caller hit it). Closes the last no-GIL gap (no C extension on the relations path — unblocking P6a's full no-GIL CI lane and P11's thread scheduler for relations) and drops the macOS `libomp` source-build, with **no new dependency**. Relation-count goldens (Fig 2/4/6/7) byte-identical. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-18-p6b-remove-graphillion*`.
...
- **B15 — `result.diff()` — landed (2026-06-16).** `a.diff(b)` on every top-level result returns a frozen, `Displayable`, `to_pandas`-able `ResultDiff`: signed Δφ (Δα for AC), a genuine-MIP-change-vs-co-optimal-tie-reshuffle flag (via `lex_key` over the tie set, which already encodes `EQUALITY_TOLERANCE`), distinctions/relations/account-links gained/lost/changed (keyed by mechanism / relata / direction+mechanism+purview), and config-diff attribution composing the landed `ConfigSnapshot.diff`. A shared `_diff_common(a, b)` computes the scalar deltas; per-type `_changes()` hooks contribute the element-level records (mirroring B8's `_findings()`/`explain()`). Comparison is pure — no recompute, no φ/α change. Cross-substrate diffs are allowed with a `substrate_note`; type mismatch raises `TypeError`; mechanism-level results and `Account` carry no `ConfigSnapshot`, so their `config_diff` is `{}` by design. One surprise vs the plan: RIA *does* expose `.ties`, so its MIP-change check uses the (more accurate) tie-set branch rather than the lex-key fallback. `pyphi/models/diff.py`; `test/models/test_result_diff.py` (coverage invariant over every result type + MIP-reshuffle invariant); plan `docs/superpowers/plans/2026-06-16-b15-result-diff.md`. Pairs with B8. *(Brainstorm B15.)*
- **B19 — CM/TPM connectivity-consistency validator (default-on) — landed (2026-06-15).** A directional, k-ary-correct check at substrate construction: `FactoredTPM.infer_cm()` (built on `infer_edge()`) infers the true edge set **per factor** — an edge `a→b` exists iff factor `b` is non-constant along input axis `a` — without materializing the joint, and `validate.connectivity()` rejects an **under-specified** CM (one omitting a real edge, which would be silently marginalized out in `get_inputs_from_cm` / `irreducible_purviews`, under-counting φ), naming the offending edge(s). **Over-specification stays legal.** Added `validate_connectivity: bool = True` to `conf/infrastructure.py` (joining the `validate_*` family), wired into `validate.substrate()`; default-on is a breaking-ish change, `validate_connectivity=False` opts out. **Key correction to the plan:** the legacy `JointTPM.infer_edge`/`infer_cm` (since deleted from `joint_distribution.py`) were **not** reused — they compare only source state 0 vs 1 (binary-only) and use exact `!=`, so they are **unsound for k-ary** substrates (the landed Gómez p53 net is ternary); B19's per-factor inference uses an absolute (`rtol=0`) precision tolerance so a small-but-real dependence is not swallowed. Cost `O(N²·factor_size)`, negligible vs φ. **Fallout — latent inconsistencies the tightened contract surfaced:** the `multivalued_k3k3_k4_sparse` golden fixture declared a 3-cycle CM but built dense random factors (self-inconsistent), so it was rebuilt with genuinely sparse factors honoring the cycle and its golden regenerated. The shipped `residue` example carries self-copying TPM columns on its input units C/D/E while its CM (matching the published diagram) omits those self-loops, marginalizing them to uniform effect repertoires — the intended input-unit behavior; a confirmation experiment showed removing the self-copy changes 896/2048 repertoires (the value depends on it), so the example is preserved exactly via a `@config.override(validate_connectivity=False)` decorator rather than altered. The reference/shape tests that deliberately pair dense factors with reduced/inconsistent CMs (`test_repertoire_reference.py`, `test_repertoire_kary_properties.py`, `test_actual.py::test_state_probability_strict_system`) opt out the same way. `substrate_generator` output stays consistent (its `cm = (weights != 0)` is always a superset of the functional edges). Self-loops are validated like any other edge (the input-unit idiom must opt out). Sibling to B13; advances "make illegal states unrepresentable." *(Brainstorm B19.)*
- **B20 — substrate ↔ networkx graph bridge — landed (2026-06-18).** `pyphi/graph.py` bridges the value types to networkx: `Substrate.to_networkx()` returns a node-labeled `DiGraph` (reusing the `nx.from_numpy_array` pattern from `visualize/connectivity.py`); `System.to_networkx()` adds per-node `state` and `in_system` attributes; `Substrate.from_networkx(graph, tpm)` reconstructs a substrate (topology from the graph, dynamics from the required TPM); `to_graphml()` and `to_adjacency()` (labeled DataFrame, P14d convention) export; and six thin networkx-wrapper topology helpers (`is_strongly_connected`, `strongly_connected_components`, `is_dag`, `simple_cycles`, `in_degree`, `out_degree`) returning node-index results. **The connectivity-inference decision is the central design point:** every graph-producing entry point defaults to `connectivity="inferred"`, using B19's `factored_tpm.infer_cm()` so that causally-inert edges the declared `cm` over-specifies are dropped — which matters because the topology questions (strong-connectivity, cycles → "is this a complex?") are only meaningful on the real causal graph. `connectivity="declared"` returns the declared `cm`. This composes with B19 in both directions: B19 guarantees `cm ⊇ inferred` (the inferred graph is always a clean subset), and `from_networkx` inherits B19's default-on rejection of an under-specified topology (the inverse footgun — silently marginalizing a real TPM edge → φ under-count). networkx loads lazily via a `DeferredNetworkX` (mirroring `DeferredPlotly`), so `import pyphi` never imports it; it stays in the `visualize` extra. Weights are out of scope (the substrate retains none — only a `substrate_generator` does), as are Bayesian-network / CPD / DBN semantics (→ N11) and igraph. `test/test_graph.py` (incl. the inferred-vs-declared phantom-edge test and a k-ary case); a lazy-import guard in `test/integration/test_lazy_imports.py`. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-18-b20-substrate-networkx-bridge*`.
- **B21 — unified object display — landed (2026-06-15).** Shipped the `pyphi.display` package: a per-type `_describe()` returning a declarative description (`Description`/`Section`/`Row`/`Table`/`Inline`/`Nested`), rendered by pluggable ASCII (boxed-card) and styled-HTML backends, with a `Displayable` mixin owning `__repr__`/`__str__`/`_repr_html_`/`_repr_mimebundle_` and the single `repr_verbosity` read site. Every user-facing result type was migrated and `fmt.py` shrank 1168→530 lines (dead composers removed; value-formatters kept). The look — vertical cards, full readable labels, decimal-consistent numbers (6 sig figs, floats keep a decimal), collections as tables, and leaf rendering for embedded values like partitions — was settled with the user at a look-lock checkpoint; the HTML backend is genuinely HTML-native (header, key/value grids, real tables) and the `rich` backend is deferred behind the backend seam. The partition-display change required decoupling the golden harness's `canonical_partition` from `str()` (now the structural `cut_matrix`); no φ values changed. Tests in `test/test_display.py` (incl. a `Displayable`-coverage invariant + an SIA ASCII golden); plan `docs/superpowers/plans/2026-06-15-b21-unified-display.md`. **Post-landing polish (2026-06-15):** configurable collection-table truncation (`repr_max_table_rows`, default 50; `… N more` indicator + scrollable HTML); RIA/MICE label consistency (φ symbol, clean specified-state, probabilities as rows); rich partition cards (a `removed_edges()`-based severed-connections cut grid, concise shorthand retained for embedding via `concise_partition`); and cause/effect color-coding in HTML (`#D55C00`/`#009E73`) across SIAs, distinctions, state specs, RIA/MICE, AC, and directed partitions (ASCII unchanged). **Further polish (2026-06-16):** table tones now apply as inline `color` styles (the class-based tone lost to the more-specific `table.pyphi-table th` rule and to some notebook front-ends' table CSS, so purview-column headers rendered gray) and the account-link table colors its direction cells per-row; and `FactoredTPM`/`JointTPM` gained a rich state-by-node matrix-grid card (scrolling/overflow for large state spaces; binary → one `P(on)` column per unit, k-ary → one column per `(unit, next-state)` pair with the full conditional, columns labeled with node names when the TPM comes from a `Substrate`) plus an optional, lazily-imported `to_xarray()` (a `Dataset` of per-unit conditionals for the factored form, a `DataArray` for the joint) — the matrix display itself pulls in no xarray. `Substrate` and `System` were promoted from one-line leaves to rich cards (Substrate: connectivity grid + embedded TPM grid; System: per-unit state, units, background, applied cut, connectivity grid, and the background-conditioned cause/effect effective TPMs over the system's units — toned cause/effect, genuinely distinct from the substrate TPM for subset systems), the `CauseEffectStructure`/`PhiFold` cards embed a relations table, the SIA cards now embed the MIP as a cut-matrix grid, and the HTML backend renders symbol subscripts (`φ_s`, `φ_d`, `Σφ_r`) as real `<sub>` (ASCII unchanged); the ASCII renderer was fixed to widen a card for section labels longer than its body. Display verbosity now gates consistently across both backends — `LOW` = compact one-liner (leaf in HTML), `MEDIUM` = card without the expensive embedded TPM grids, `HIGH` (default) = everything — so a `System`'s conditioned cause/effect marginals (and a `Substrate`'s TPM grid) are computed only at `HIGH`. The RIA/MICE cards were reorganized into labeled sections (summary, MIP, Ties, Repertoire), the repertoire now renders as a styled distribution table (`Pr`/`Pr (cut)` columns) rather than ASCII, and a new top verbosity level `FULL` (3) adds the partition's cut-matrix grid to those cards and embeds the `SystemIrreducibilityAnalysis` flat into the `CauseEffectStructure` card (its sections appended as CES sections, consistent with how every other card embeds a sub-object — no nested boxes). **Original plan:** Object formatting has accumulated into spaghetti: one 1169-line `pyphi/models/fmt.py` (~50 flat `fmt_*` helpers, Unicode box-drawing, the `align_columns` workhorse), text-only, with `_repr_html_` on just 4 classes (the SIAs, `CauseEffectStructure`, `AcSIA`) via a bare unstyled `html_columns` table — every other result type has no notebook rendering. Delegation is inconsistent (`Relation`/`RelationFace`/`AcSIA` hand-roll their own boxed reprs bypassing `fmt`; some `__str__` return `repr(self)`; some define no `__str__`), and `repr_verbosity` is read in scattered places. Replace this with a **structured-description → multi-backend-renderer** pipeline: (1) each result type implements one `_describe()` hook returning a declarative description from a small layout vocabulary (sections, key-value rows — the generalized P11.87 `fmt_*_columns` pattern — side-by-side panels, tables, nested sub-descriptions), the single source of truth for *what* to show; (2) a renderer with pluggable backends turns it into output — **ASCII/Unicode** (always available; today's `fmt.py` primitives `box`/`header`/`align_columns`/`side_by_side`/`indent` survive as renderer internals), **styled HTML** for notebooks (replacing the bare table), and an **optional, lazy `rich`** backend (a `DeferredRich` mirroring `pyphi/deferred/deferred_import.py`'s `DeferredPlotly`) that renders `rich.Table`/`Panel`/`Tree` for styled terminal *and* notebook HTML in one path, degrading to ASCII/HTML when absent — no mandatory dependency; (3) a shared `Displayable` mixin supplies `__repr__`/`__str__`/`_repr_html_`/`_repr_mimebundle_` dispatching to `_describe()` + the active backend, removing per-class duplication and the `__str__`→`repr(self)` circularity; (4) `repr_verbosity` read in exactly one place. **Coverage:** every user-facing result type (3.0/4.0 SIAs, `CauseEffectStructure`/`PhiFold`, `Distinction`/`Distinctions`, `RIA`, `MICE`, `Relation`/`RelationFace`/`Relations`, partitions & cuts, `Complex`/`ExcludedCandidate`, `System`/`Substrate`/`Node`, `AcRIA`/`AcSIA`/`Account`, repertoires, state specs); the ad-hoc reprs and `DirectedBipartition` special-casing all route through the model. **Out of scope:** `pyphi/visualize/` (matplotlib/plotly *figures*) — a separate heavy-optional-dep concern, not inline object display. **Coordinated with P14d** (`to_pandas` consolidation) — the description model and the pandas export share one labeled-field extraction, though P14d owns the pandas surface. **Foundational for B8** (`result.explain()`) **and B15** (`result.diff()`) — both want `_repr_html_`/display and should plug into B21 rather than grow their own renderers. **Pre-freeze** (Wave 2): it defines the public repr/HTML surface P15 freezes. **Load-bearing risk:** many doctests across `pyphi/` and `docs/` plus `test/models/test_result_protocols.py` assert on repr/HTML content — the ASCII backend must reproduce current text output, or affected doctests/goldens are updated deliberately and reviewed as intended surface changes, not silently. Verify with `uv run pytest` (no path argument, so the `pyphi/` doctest sweep runs). *(Brainstorm B21.)*

### Wave 3 — Regression gate

- **P11.8 Tier 2 — benchmark rewrite + ASV-in-CI — landed (2026-06-22).** The `benchmarks/` suite predated 2.0 and no longer imported (`pyphi.Subsystem`, `pyphi.compute`, `BenchmarkConstellation`); the five museum modules were deleted and the suite rebuilt on the golden harness. A shared `test/golden/perf.py` holds the cProfile counting primitive, the canonical hot-frame list, and the fixture/grain dispatch, imported by the gate, the regen script, and the ASV suite (via a `sys.path` shim in `benchmarks/benchmarks/_fixtures.py`) so they cannot drift. **Wall-time (advisory, nightly):** `layers.py` benchmarks every golden fixture × {repertoires, mechanism_mips, phi_structure, sia}; `edges.py` (parallel-vs-sequential, cold-vs-warm cache), `micro.py` (EMD kernel), `actual_causation.py` (AC account); `asv.conf.json` points at `main`; `.github/workflows/benchmark.yml` caches results across runs via `actions/cache`, runs new commits, and opens/updates a tracking issue on a detected regression (the check compares HEAD~1 vs HEAD only, so multi-commit drift is not detected) — never on PRs. **Counts (deterministic, blocks PRs):** `test/integration/test_perf_counters.py` asserts exact pinned call counts (`test/data/perf/call_counts.json`, regenerated by `scripts/gen_perf_counts.py`) for a bounded subset (each formalism × layer + k-ary + relations-heavy + AC); `counts.py` tracks the full zoo's counts nightly as ASV `track_*` metrics. The cProfile counts are exact and add no production overhead. *Moved ahead of P13 SP2 / P17 / P18* — it is the regression gate those hot-path items need (the class of gate that would have caught the 60–300× YAML-write slowdown). Supersedes P15 "Layer D".

### Wave 4 — Internal cheap wins *(anytime after their deps)*

- **P9.5 — math-fingerprint cache keys — landed (2026-06-27).** Label-free `blake2b-256` content fingerprints replace object identity as the cache key. `System._fingerprint` digests exactly the components `System.__eq__` compares (alphabet sizes + factor bytes + cm + state-as-indices + node/external indices + partition `removed_edges`), omitting `node_labels`/`state_space` labels; `Substrate._cm_fingerprint` digests connectivity alone. A reusable refcounted `ContentCache` (`pyphi/cache/content.py`) keys the repertoire kernel cache on the System fingerprint and `potential_purviews` on the cm fingerprint — so equivalent/relabeled systems and same-topology TPM-weight sweeps reuse results — and evicts a fingerprint's entries when its last live carrier is GC'd (prompt release preserved, plus cross-object reuse). A 256-bit digest (no slower than 128 under blake2b, <0.1% key overhead) keeps the accidental-collision probability below hardware-error rates. Surfaced and fixed a latent `MacroSystem` collision: it overrides `cause_marginal` to a separate field, so the inherited fingerprint conflated distinct groupings — `MacroSystem` now digests both derived marginals the kernel reads. Hypothesis invariants (relabeling collides + agrees; math difference separates), byte-identical goldens, and an unchanged perf-counter gate. Unblocks N4 (the disk cache reuses the digest). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-27-math-fingerprint-cache-keys*`.
- **P6a — free-threaded CI lane + concurrent cache safety — landed (2026-06-27).** A blocking Python 3.14t lane runs the full suite with `PYTHON_GIL=0` (the suite is green free-threaded — 2790 passed — so no xfails; 3.14t is the target because `msgspec` ships free-threaded wheels for `cp314t` only, and the lane installs extras + pytest directly because the dev group's graphify tooling has no free-threaded wheel). The content cache is now thread-safe: `ContentCache.evict` iterated the live dict and crashed under the thread scheduler (`RuntimeError` swallowed in a weakref finalizer, eviction lost) — a reentrant lock now guards eviction and refcount bookkeeping with an atomic key snapshot, while the hot path stays lock-free and the hit/miss counters stay best-effort. A standard-lane guard pins thread-backend SIA ≡ sequential SIA. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-27-p6a-nogil-ci*`.
- **Logging rework — silent by default — landed (2026-06-28).** PyPhi configured the **root** logger on config load and, with `log_file_level` defaulting to `"INFO"`, wrote a growing `pyphi.log` into every working directory — application behavior in a library. It now attaches a single `NullHandler` to the `pyphi` logger at import and does nothing else: no root-logger configuration, no console output, no file. Logging is turned on explicitly with `pyphi.enable_logging(level="INFO", file=None)` (a progress-bar-safe `TqdmHandler` on stderr, or a `FileHandler` when `file` is given; repeat calls replace and close the prior handler; `propagate=False` so records do not also flow into a host application's root handlers). The three `log_file` / `log_file_level` / `log_stdout_level` config options, their validators, and the `configure_logging` callback were removed; logging is no longer set through `pyphi_config.yml` (a breaking change for 2.0). Because the config snapshot and routing derive from the dataclass fields, the options also dropped out of the reproducibility snapshot automatically — correct, since logging is operational, not result-affecting. The `warnings`/`PyPhiWarning` system is independent and untouched. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-28-logging-rework*`.
- **P11 — cluster backends — landed (2026-07-21).** `DaskScheduler` implements
  the full Scheduler Protocol against a user-connected `distributed.Client`
  (spec `docs/superpowers/specs/2026-07-20-dask-cluster-backend-design.md`), and
  the HTCondor-native batch surface landed as `pyphi.campaign` (sweep campaigns
  + scoped CES sharding with exact tie-preserving merges; smoke-tested at
  production scale on CHTC 2026-07-23). See the dashboard row for details.
- **Parallel engine unification — landed (2026-06-23).** Two coexisting map-reduce implementations were collapsed into one flat path: the one-shot `MapReduce` class was replaced by a `pyphi.parallel.map_reduce()` function over the Scheduler Protocol, all 10 call-sites migrated, the vestigial execution tree (`tree.py`/`test_tree.py` + the dead `branch_factor`/`max_*` kwargs and their config allow-list entries) deleted, and the ~5 duplicated helpers deduped. `config.parallel_backend` (thread/dask) is now honored from the public entry, where the class always used the process pool. Net ~900 lines removed; result-preserving (N2 + goldens + perf gate). The Scheduler-facade migration the Protocol was built for in P11 but never finished. Prerequisite for B18. Spec/plan `docs/superpowers/{specs,plans}/2026-06-23-unify-parallel-engine*`.
- **ASV nightly build fix — landed (2026-06-23).** The managed-environment build path the P11.8-T2 nightly workflow depends on had never run and was broken three ways: hatch-vcs failed to derive a version in a git-less build (`fallback_version`), asv's default `pip wheel -w` stranded the wheel in pip's cache (`build_command` → `python -m build -o`), and the managed env omitted the `emd` extra so the EMD microbenchmark failed to import (`install_command` → `[emd]`). All three fixed and verified end-to-end with a persisted local baseline.
- **B18 — adaptive parallel granularity — landed (2026-06-23).** Activated `ChunkingPolicy.size_func`. A pure `cost_balanced_partition` (`pyphi/parallel/chunking.py`) LPT-packs items into cost-balanced chunks by a cheap parent-side `size_func` weight, and the chunk count is floored at `num_workers`. **The floor is a general core-utilization win** (it applies on the default even-count path too): the coarse config chunksizes leave cores idle on medium workloads, and a one-off de-risk experiment confirmed flooring is a 2.8–3× speedup at every per-item cost with no overhead penalty in the realistic range — measured, not assumed. Cheap cost proxies wired at the heterogeneous sites: relations (`overlap×degree`), purview eval (`2^|purview|`), concept eval (`2^|mechanism|`). `size_func` is incompatible with `ordered=True` (it reorders items) and raises. Result-preserving (N2 incl. a forced-tiny-chunk concept test that exercises packing end-to-end, goldens, perf gate); a Hypothesis property test caught and fixed a mismatched-iterable-length bug. ROADMAP cost-formula note: the original `|potential_purviews| × alphabet-product` mechanism estimate was simplified to the cheap `2^|mechanism|` proxy (only relative ordering matters; N2 makes any proxy correctness-safe), and the homogeneous mechanism-partition site (`queries.py:118`) was intentionally skipped. Spec/plan `docs/superpowers/{specs,plans}/2026-06-23-cost-balanced-chunking*`.

- **B18 dispatch-gate follow-up — landed (2026-07-07).** The dispatch gate previously ran any workload at or below one `chunksize` sequentially, so the `num_workers` chunk-count floor never applied below that boundary (with the relations chunksize of 4096, 4,000 candidates ran fully sequential while 4,097 fanned out). A benchmark study (`benchmarks/b18_dispatch_gate.py`, raw results in `benchmarks/b18_dispatch_gate_results/`) measured both gates across the affected regime at six real map sites and found the profitable boundary is total estimated *work*, not item count: warm-pool parallel dispatch wins 3–4× for ~ms-scale items below one chunksize (purview MIPs, system partitions, complex SIAs — including reducible short-circuiting systems) and loses only when total work is tens of ms (µs-scale lazy relation construction, ~50 µs mechanism partitions). The gate now parallelizes whenever the chunker would produce more than one chunk: `sequential_threshold` is the sole dispatch gate, an explicit `chunksize` governs granularity only, and a cost-sampled chunksize still gates dispatch (it estimates the items per ~1 s of work, preserving the cost-aware sampled path). Per-level threshold defaults retuned to measured per-item costs: `parallel_partition_evaluation` 1024→64 (also unblocks IIT 3.0 cut sets, all below 1024 for n≲11), `parallel_mechanism_partition_evaluation` and `parallel_relation_evaluation` 1024→8192. The study's first run was confounded by a per-item `hash(repr(snapshot))` (~1.3 ms/item) in the process-scheduler worker wrapper — now computed once at wrap time, which alone made sequential relation-candidate evaluation ~250× faster; the gate decision rests on the post-fix data.

### Wave 5 — Surface freeze (P15, last surface-affecting work)

**Import-surface cleanup + B17 — landed (2026-06-24).** Retired the eager
`_import_submodules` walk (and its `_skip_import` hand-list): registries are now
populated by explicit imports of the seven registrant modules (the audit
confirmed all 53 decorator registrations are co-located with the registry they
populate), and public submodules resolve through a PEP-562 module-level
`__getattr__` that imports them on first attribute access — so `import pyphi` is
faster, no longer imports the whole tree, and no longer fails when an unrelated
submodule fails to import. A registry-contents test pins every registry's keys
so a dropped registrant fails loudly. **B17** landed in the same pass: removed
the unused `tblib`; deleted the unused `OrderedSet`/`HashableOrderedSet` types
(a sorted tuple is the better representation for units in purviews/mechanisms)
and dropped `ordered-set`; migrated `toolz` to `itertools.chain.from_iterable`,
`more_itertools.unique_everseen`, and `functools.partial`. Spec/plan
`docs/superpowers/{specs,plans}/2026-06-24-import-surface-cleanup*`.

**jsonify → msgspec serialization — landed (2026-06-25).** A typed `msgspec`-based `pyphi.serialize` package replaces the custom `pyphi/jsonify.py` JSON layer, which is **deleted** along with every per-class `to_json`/`from_json` hook (58 methods) and the tie-peer serialization contextvars. The new package: one frozen `msgspec.Struct` per result type in a tag-discriminated union (`serialize/schema.py`), centralized `to_schema`/`from_schema` converters keyed by type (`serialize/convert.py`), numpy arrays stored as their `.npy` bytes (`serialize/arrays.py`; base64 in JSON, raw in msgpack, `allow_pickle=False`), and a `dumps`/`loads` (bytes) plus `save`/`load` (path or file object, format inferred from the extension) API over a single top-level `format_version` with both JSON and msgpack from one schema. The cause-effect-structure schema is **normalized**: distinctions are stored once in a table and each relation references its members by index, removing the dominant redundancy of embedding every distinction in every relation. Tie-peer graphs (RIA, SIA, AcRIA, state-spec, CausalLink) are reconstructed on decode. Validated two ways: ~94 synthetic round-trip tests (every type, JSON + msgpack) and a one-time migration that loaded all 27 jsonify-format reference fixtures the suite consumes with the legacy serializer and asserted the new serializer round-trips each to an equal object in both formats — the migration's output is the committed new-format goldens (the old jsonify format remains recoverable from git history). Measured compaction on the rule154 phi-structure: 1.31 MB → 56 KB JSON (23×), 43 KB msgpack (31×). It is a **format break**: results saved in the old jsonify format must be recomputed (no standalone converter ships). The ergonomic loading surface also landed: path-based `serialize.save`/`load` with format inferred from the extension, top-level `pyphi.save`/`pyphi.load`, and `.save()`/`.load()` methods on the user-facing result types via a delegation-only `Serializable` mixin (`pyphi/serializable.py`); type registration is deferred to first use (`_ensure_registered`) so importing `pyphi.serialize` performs no domain imports; the redundant `substrate.from_json` loader was removed. *(Config-as-Struct and domain-level generative-relations compression stay deferred; the dead `validate_json_version` config option was subsequently removed.)* **`Analysis` registered (2026-08-03):** the type `pyphi.analyze()` returns was never given a schema, so the object a newcomer actually holds could not be saved — only its `.ces` component. It now round-trips under all three formalisms (the `ces` field is a union, since IIT 3.0's cause-effect structure is the bare distinction sequence) and carries `Serializable`; the save/load how-to no longer demonstrates the failure.

**Test suite reorganized (2026-06-26):** `test/` now mirrors the `pyphi/` package layout — subpackage tests under `test/<subpackage>/`, cross-cutting/paper/golden/invariant tests under `test/integration/`, root-module tests at `test/` root, shared infrastructure (conftest, `example_*`, `golden/`, `data/`, `reference/`) unmoved. Pure relocation (four opaque filenames clarified); splitting mixed-formalism files (e.g. `test_big_phi.py`) was intentionally declined — relocation only, no re-carving. **Remaining P15:** the comprehensive **docstring sweep** (rewrite to final-state voice — project-stage markers already stripped and a keyword-driven narrative-removal slice landed; the exhaustive pass is folded into the [Documentation overhaul](#documentation-overhaul-20--iit-40) project); condense changelog fragments to first-encounter voice. `to_pandas` extend = **B21** (done). Open-PR triage **done** (2026-06-26 audit: every fix PR already resolved in 2.0, rest moot/superseded — see [Open PRs to Absorb](#open-prs-to-absorb); GitHub closes pending a maintainer).
...
- **P17 — perf characterization (Wave A complete; Wave C closed; Wave B deferred out of 2.0, 2026-07-12).** Extend the cross-temporal benchmark beyond 5 nodes, find the interactive/batch size thresholds, characterize the mechanism behind the IIT 4.0 speedup, and exercise the 2026 ii-cap with non-zero φ. **Correction (2026-06-26):** the earlier "19–43× IIT 4.0 speedup" was a measurement artifact — it compared pre-refactor `phi_structure` (full structure) against post-refactor `sia` (system-φ only). The de-confounded, like-for-like result is a **~18–20× faster SIA partition evaluation and ~2× faster CES**, driven by the `core/repertoire_algebra.py` kernel rewrite; the cap variant is computationally free. The hot-path config-behavior sweep (`config_sweep.py`) found no documented-versus-actual mismatch and confirmed both pre-2.0 config bugs are fixed (`PARALLEL=False` now truly disables subprocesses via the `parallel_kwargs` global gate; incompatible version/measure combinations raise a clean `ConfigurationError` instead of a raw crash). A seeded 6–7 node Ising generator (`harness._synth_system`, ferromagnetic coupling so the all-off state integrates and the SIA runs the full search rather than short-circuiting) plus committed fixtures land the Part 3 construction; n=6 sizing is ~24 s/trial profiled and the partition count grows ~7.8×/node, so n=7 is batch-only and deferred to the lab server. Wave C (one targeted optimization) is closed with a negative result — no bounded ~1-week win, since the ~18× gain is already realized by the repertoire-algebra rewrite (the remaining lever is the larger N5 Rust-kernel item). The Wave B 6–7 node server matrix is **deferred out of 2.0** (measurement-only, blocked on lab SSH; see the deferred table). `benchmarks/iit_3_vs_4/README.md`.
...
- **Web-docs tooling overhaul.** ✅ Landed. The Sphinx build itself was
  modernized — pydata-sphinx-theme + myst-nb, a build-time recursive
  autosummary API reference against the 2.0 module layout, executable MyST
  pages, doctest-in-CI, and a fixed readthedocs build — so the site is
  maintainable, not just rebuilt once.
- **Non-docstring narrative content, current for 2.0 and 4.0.** ✅ The IIT 4.0
  **theory narrative landed**: an eight-page section under `docs/theory/`
  (distinctions, relations, system integration, the
  `Substrate → System → formalism → φ-structure` layering, the paper-to-code
  map, formalism versions, conditional independence, IIT 3.0), computation-driven
  and grounded in one build-time-executed worked example, with equation
  citations to Albantakis et al. 2023 and the 2026 ii-cap paper all
  paper-verified. Design/plan:
  `docs/superpowers/specs/2026-07-08-theory-narrative-design.md`,
  `docs/superpowers/plans/2026-07-08-theory-narrative.md`. Task-focused usage
  walkthroughs — the **tutorials & how-tos sub-project** — ✅ landed: a
  getting-started walkthrough, five tutorial pages (cause-effect structure, a
  complete XOR worked example, macro, actual causation, and the paper-supplement
  notebook), and seven how-to guides, all executable MyST verified at build
  time. Design/plan:
  `docs/superpowers/specs/2026-07-08-tutorials-howtos-design.md`,
  `docs/superpowers/plans/2026-07-08-tutorials-howtos.md`.
- **Example notebooks as a curated teaching set.** ✅ Landed.
  `docs/examples/IIT_4.0_demo.ipynb` — the published IIT 4.0 paper supplement —
  is faithfully updated to the 2.0 API in place at its exact path (so the
  paper's Colab link still resolves), reproducing the paper's numbers on the
  paper's own example systems, and is surfaced through the Tutorials section by
...
- **Migration guide.** ✅ Landed. `docs/migration/migration-2.0.md` documents
  ...
  serialization format break), topic-organized with [1.x]/[4.0-branch]/[both]
  audience tags and verified 2.0 snippets. Design/plan:
  `docs/superpowers/specs/2026-07-08-migration-guide-design.md`,
  `docs/superpowers/plans/2026-07-08-migration-guide.md`.
- **Final-state docstring sweep.** ✅ Landed. Every `pyphi/**` docstring
  rewritten in final-state voice — what the thing *is*, not what it was or how
  it was built — in uniform NumPy style (Google sections converted,
  `napoleon_google_docstring = False` enforcing it), with substitution markup
  replaced by Unicode symbols and paper-verified literature citations added.
  Ran as a 20-batch two-stage (rewrite + independent accuracy-verify) pass.
  Code-level bugs and unconfirmed docstrings it surfaced are recorded in
  `docstring-sweep-findings.md` for separate follow-up (not fixed in the sweep).
  Design/plan: `docs/superpowers/specs/2026-07-08-docstring-sweep-design.md`,
  `docs/superpowers/plans/2026-07-08-docstring-sweep.md`.

All five sub-projects have landed, so this overhaul is complete. It absorbed the
docs bullets that were scattered through P15 (the "Remaining P15" line, the
archive's "Sphinx site rebuilt" / "docstring cleanup pass" / "migration guide" /
"demo notebook rewrite") and is the home for ship-criterion #5, now discharged.
...
New ideas surfaced by the 2026-06-13 audit (the roadmap "started as an engineering overhaul and wishlist", so formalism-level and algorithmic ideas are in scope). **Most have since landed (N1, N2, N4, N7, N8, N9, N11 — see the dashboard and the per-item annotations below).** The rest are candidates for 2.x or for folding into the waves above.
...
**Follow-up — k-ary support for the matching/triggered-TPM path** *(recorded 2026-07-16 by the precondition-guards fix; open)*. The clamp-then-noise construction (`pyphi/matching/triggered_tpm.py`) is built on the binary state-by-node representation — the ON-probability slice, `(2,)*n` step-TPM shapes, the binary state-by-state conversion, and little-endian state decoding — so non-binary substrates are rejected at the entry points (`build_triggered_tpm`, `PerceptualSystem`) as a current implementation limit, not a theoretical one. Generalizing requires reworking the construction onto explicit-alphabet per-node distributions and mixed-radix state indexing (the k-ary primitives exist in `pyphi.core.tpm` and `pyphi.utils`), plus confirming the matching formalism's definitions carry over unchanged.
...
- **B8** — `result.explain()`. *Landed (2026-06-16); see the dashboard row + archive.* Typed `Explanation` tree of findings (which short-circuit fired; for φ>0 the winning/runner-up partition and φ-gap on system SIAs, the binding min direction, the driving mechanism/purview) on every top-level result.
...
- **B12** — `pyphi.Sweep` cartesian batch driver. *Promoted to `2.0` (see dashboard).* `sweep(substrate, states=..., subsets=..., formalisms=[...])` → a `SweepResult` with one tidy MultiIndexed DataFrame (`.df`) (φ, n_distinctions, Σφ_r, version) with raw results in `.results` and a `ConfigSnapshot`/seed per row. Co-design the row schema with P14d `to_pandas`.
...
- **B15** — `result.diff(a, b)`. *Landed (2026-06-16); see the dashboard row + archive.* Structured `ResultDiff` (Δφ, genuine-MIP-change vs tie-reshuffle, distinctions gained-lost keyed by mechanism; relations gained/lost by full-object matching, so a φ-only change on a relation shows as one lost + one gained) composing `ConfigSnapshot.diff`; `Displayable` + `to_pandas()` on every top-level result.
...
- **N18 — Tractability pre-flight / cost oracle — landed.** Both slices have landed: the
  grain-search cost pre-flight (`SearchBounds.estimate`, 2026-07-10; see
  `docs/superpowers/specs/2026-07-10-grain-cost-preflight-design.md`) counts what a
  `pyphi.macro.complexes` sweep would evaluate with no TPM construction, and the single-system
  pre-flight (`pyphi.estimate_analysis` in `pyphi/cost.py`, 2026-07-20) counts the full
  φ-structure workload — system partitions, purview evaluations, mechanism partitions — for a
  full, SIA-only, or CES-only analysis, scope-aware and state-independent, with a budget cap on
  the counting walk itself. The MCP `analyze` guard and the campaign admission checks consume it.
...
- **N22 — Intrinsic differentiation / specification as first-class quantities.** ✅ **Landed
  (2026-09, 2.0.0 release pass):** `intrinsic_specification` / `intrinsic_differentiation` /
  `intrinsic_information` / `integrated_fraction` on the IIT 4.0 SIA (and the four per-direction
  `to_pandas()` columns); `explain()` names the direction and term that set φ_s when the
  requirement binds (`requirement_binding`); the determinism sweep is pinned as the Mayner et
  al. 2026 Fig 3 reproduction (`iit4_2023_fig6d_substrate(k=...)`), the monad as Fig 2. The
  card labels the per-direction row by the formalism's own name for the quantity. *Earlier
  notes:* the grain exploration's ii-decomposition
  ...
  margin −0.054 (`experiments/ii_phi_inequality_experiments/FINDINGS.md`); it holds by construction only
  under the 2026 Eq. 23 requirement, so a suite-wide `φ_s ≤ ii` assertion must not be added.
- **N23 — Φ-explanatory diagnostics.** *`2.x`; partially landed.* The selection-margin
  reporting (dashboard row), `Complex.exclusion_margin`, and the `pyphi.landscape`
  sections/`switch_distances` are landed concrete slices; the substrate-landscape exploration
  (`docs/superpowers/specs/2026-07-07-substrate-parameter-landscapes.md`)
  established margins as the IIT-native sensitivity analysis; the integrated-fraction ratio
  landed as `integrated_fraction` (the Marshall et al. 2023 Fig 2 reproduction pins the
  paper's φ_c / ii_c percentages). Remaining: determinism/degeneracy covariates, the φ_s
  landscape display. Original scope: surface the Marshall et al. 2023 system-φ
...
an intrinsic-vs-extrinsic tie must be discriminated) — *landed 2026-09* as
`test/formalism/test_complexes.py::TestS1TerminalTieBranch`, a mirror-symmetric substrate whose
tied pair fails exclusion end to end through `Substrate.complexes`; and Marshall's **Theorem 1**
...
### 2026-07-22 upstream fixes from the first production-scale campaign

> The first overnight scoped-CES campaign at production scale (a 21-unit dense
> substrate, a 1,372-mechanism binding scope) hit four concrete problems in pyphi.
> **All four landed 2026-07-22** (spec
> `docs/superpowers/specs/2026-07-22-upstream-campaign-fixes-design.md`). Two change
> on-disk campaign formats, so campaigns must be re-prepared (the version guard
> refuses cross-version collects).

- **✅ Purview order cap.** `Substrate.potential_purviews(max_order=...)` bounds the
  enumeration itself (powerset `max_size`) instead of filtering afterward; the purview
  cache key includes the bound so capped and uncapped results never alias. Scoped
  consumers derive the bound via `AxisScope.order_bound()` (single selection point
  preserved), and explicit purview lists bound mechanism-level queries. At 21 units:
  ~2M candidate constructions per (direction, mechanism) → hundreds; planning walks
  that did not finish in 8 minutes complete in seconds, with identical results
  (bounded ≡ unbounded-then-filtered, asserted on dense and sparse connectivity).
- **✅ Substrate serialization at raw-array size.** Campaign substrates are stored as
  `substrate-<label>.msgpack.gz` (raw bytes, no base64), and serialized binary TPM
  factors store only the on-probability slice when the off slice is its exact float
  complement (verified elementwise at encode; `FORMAT_VERSION` 2). A dense sigmoid
  substrate serializes at ~1.0× its raw state-by-node array (~0.94× gzipped) versus
  ~1.95× before — at 21 units, 641 MB → ~330 MB. Round-trips are bit-identical,
  pinned on sigmoid-probability and untrimmable-factor substrates in both wire
  formats.
- **✅ Per-cell resolution states.** Multi-cell `prepare_ces` accepts
  `resolution_state` as a cell-keyed mapping, a state-keyed mapping (singleton other
  axes), or a callable; values are validated at prepare time (wrong types no longer
  fail deep inside collect). SIA shards are suppressed per cell, states are stored
  per cell (`resolution_state-<cell>.json.gz`), and collect resolves each cell
  against its own state — one campaign spanning many states plans once instead of
  once per state. `collect`'s override accepts the same forms; `sia` stays
  single-cell.
- **✅ Specified-state computation scaling.** `intrinsic_information`'s composite path
  finds winner and tie family by vectorized argmax over the measure's array (Fortran
  ravel matches `all_states` enumeration order) with tolerance ties via the new
  `numerics.eq_mask`; semantics pinned against the enumeration-based reference,
  including tolerance ties where the winner precedes the raw argmax.
  `unconstrained_forward_effect_repertoire` accumulates a running mean (memory: one
  repertoire instead of 2ⁿ — hundreds of TB at 21 units), raises with an estimate
...
- **PR #138 (substrate modeler)** (+3577/-452): Adds a `substrate_modeler/` subpackage
  making substrates stateless, unit states as ints, and removing BaseUnit. The
  value-type half was absorbed by P7/P12; the **mechanism library** half has now
  landed natively in `pyphi.substrate_generator` (see the dashboard's
  "substrate_modeler mechanism library" entry — golden-faithful to the original,
  matching substrate built natively). PR #138 is fully superseded.
...
- *Files:* New `test/data/golden/*.npz`, new `test/integration/test_golden_regression.py`. Modifies
...
> (`test/integration/test_invariants_hypothesis.py`, `test/hypothesis_utils.py`). Stale
...
- *Files:* Expand `test/hypothesis_utils.py`; add `test/integration/test_invariants_hypothesis.py`.
...
overhaul). Test `test/integration/test_lazy_imports.py` pins the deferred-import
contract.
...
**P9.5. Math-fingerprint cache keys (cross-label cache reuse)** — *landed
(2026-06-27); see the dashboard row and the Wave 4 summary for the final shape.
The historical design sketch below is retained for context.*
...
`System._fingerprint` cached property (content-hash of factor bytes
...
the descriptor-pattern tests in ``test/conf/test_config.py`` and the
...
``test/integration/test_perf_budget.py`` (Tier 1), ``.github/workflows/`` (new
...
10. **Macro framework — Marshall 2024 intrinsic units.** In scope for
    ...
    - **SP2 — intrinsic-unit criteria and search** *(landed; spec
      ...
      $f(U^J, W^J)$ subset-semantics question. Also queued: Eq. 19
      read literally diverges from the recursive identification the
      same paper describes for Eq. 8 (and from Marshall 2023
      Alg. A1) on chain topologies, where an already-excluded
      candidate would veto a third, disjoint system; the recursive
      reading was ruled canonical and is what the driver implements
      (see the Complex-unification dashboard row).
    - **SP3 — reference goldens** *(landed; spec ``9cc97a2``, plan
      ...
      as regression goldens in ``test/macro/test_macro_goldens.py``:
...
17. **P11.95c — Substrate canonicalization for intrinsic equivalence.**
    ...
    **RESOLVED (2026-06-13, Wave-1 CES-completeness search;
    ``test/integration/test_ces_completeness.py``).** The intuition that the CES
...
- *Files:* `pyphi/formalism/iit4/bounds.py`, `test/integration/test_bounds.py`,
  `test/integration/test_bounds_reference_golden.py`, `test/data/bounds/reference_goldens.json`.
...
is live (`pyphi/macro/system.py`) and `test/macro/test_macro_system.py` is active.
...
- *Status:* **Landed** (SP1 + SP2 + SP3 + search parallelization). `MacroSystem` is a live,
  exported class (`pyphi/macro/system.py`); `test/macro/test_macro_system.py` (330 lines) is active.
...
- *Files:* `pyphi/macro/` package (`units.py`, `tpm.py`, `system.py`, `criteria.py`,
  `search.py`); `test/macro/test_macro_system.py` (330 lines, active). The legacy
...
> **A-2 (3-D hypergraph view, then named simplicial-complex) — done:** rebuilt on the projection
> (`view="hypergraph"`; endpoint/face projection, shell geometry,
...
**Status (negative result, 2026-06-27).** Task 1 ran and the bottleneck does
not bind, so tasks 2–5 were not pursued. On a seeded sparse Ising substrate
(ferromagnetic, density 0.30), a full IIT 4.0 SIA at `n=7` spends ~94% of its
time in the `DIRECTED_SET_PARTITION` search; `_cause_marginal_factored` is ~6%
(mostly downstream `FactoredTPM` construction, the dense `pr_joint` itself
sub-millisecond). Because the partition count grows ~7.8×/node against the
joint's `aⁿ` (2×/node binary), the inversion's runtime share shrinks toward
zero with `n` — 6% at `n=7` is the high-water mark. The one truly `aⁿ`
representation cost is substrate construction (full-dimension factors), which is
milliseconds at every tractable size and only reaches seconds near `n=16`,
where the partition search is already intractable. B6's effect-side
generalization does not change the conclusion: the dominant cost is the *number*
of purview-sized repertoire evaluations the cut lattice demands — not any single
marginal's dimensionality — and low treewidth does not make the
treewidth-independent partition search tractable. Building junction-tree
inference would touch a correctness-critical hot path to shave a shrinking ≤6%
while the substrate-size ceiling stays pinned by the partition combinatorics, so
it was not built. If a future need arises for large-sparse-substrate repertoire
queries *outside* the Φ partition search, the lever would be the
reduced-dimension factor representation (task 3), not the cause inversion alone —
a separate research item, not 2.0 scope.
...
- **Layer D — Performance regression** (~~P15~~ — **superseded by P11.8 Tier 2,
  landed 2026-06-22**). ASV runs nightly; deterministic cProfile call counts gate
  PRs. Tells you when a refactor silently broke memoization.
