...
| N8 provenance stamp | ✅ landed | 2 | `Provenance` record (`pyphi/provenance.py`: pyphi version, git sha + dirty flag, ISO-UTC timestamp, wall-time, optional seed, Python/numpy/scipy versions, platform) auto-stamped as a sibling `provenance` field alongside the P10 `config` snapshot on the four top-level result types (IIT4 SIA, IIT3 SIA, CES, AcSIA). Wall-time filled at the `sia`/`ces` compute entry points; equality stays φ-based so provenance never pollutes comparisons, diffs, or goldens; jsonify round-trips it. Renders at a new top display tier `PROVENANCE` (`repr_verbosity=4`). Users can re-stamp their own `note`/`seed` via `result.with_provenance(...)`. |
...
| P18 (+B6) | ✖️ negative result | 6 | Confirmation experiment (task 1) ran: the dense `aⁿ` cause-inversion does **not** bind — the partition search is ~94% of a full SIA and the inversion's share shrinks with `n`. No junction-tree inference built. See the P18 negative-result note in Remaining 2.0 Work |
...
- **P18 (+B6) — sparse / treewidth-dispatched exact inference — negative result (2026-06-27).** Task 1 (confirm the bottleneck binds) ran and returned negative: the dense `aⁿ` cause-inversion does not bind, so no junction-tree inference was built. Measured on a seeded sparse Ising substrate (ferromagnetic, density 0.30), a full IIT 4.0 SIA at `n=7` spends ~94% of its time in the partition search (`evaluate_partition` over the full `DIRECTED_SET_PARTITION` set, driving ~10⁶ purview-sized repertoire evaluations); `_cause_marginal_factored` is ~6% and mostly downstream `FactoredTPM` construction, with the dense `pr_joint` itself sub-millisecond. The partition count grows ~7.8×/node while the joint grows `aⁿ` (2×/node for binary), so **the inversion's share shrinks toward zero as `n` grows** — 6% is the high-water mark. The only genuinely `aⁿ` representation cost is substrate construction (full-dimension factors), which is milliseconds at every tractable size and reaches seconds only near `n=16`, where the partition search is already astronomically intractable. B6's generalization (junction-tree effect repertoires) does not change this: the dominant cost is the *number* of small purview-sized repertoire evaluations the cut lattice demands, not any single marginal's dimensionality, and low treewidth does not make the (treewidth-independent) partition search tractable. A junction-tree rewrite would touch a correctness-critical hot path to shave a shrinking ≤6% while leaving the substrate-size ceiling pinned by the partition combinatorics. *(This was the sole "P18"; the cluster-backend deferral that once shared the label is folded into Wave 4 / P11.)*
...
- **B6** — Junction-tree / treewidth-dispatched exact inference. *`research`.* Compile per-node factors into a junction tree; compute repertoire marginals in `O(2^treewidth)` via belief propagation, covering effect repertoires + unconstrained-forward averages. Generalizes P18 (cause-inversion only). **Subsumed by the P18 negative result (2026-06-27):** the dense joint does not bind the Φ pipeline — the partition search dominates and is treewidth-independent — so junction-tree marginals do not move the binding constraint. Only revisit for large-sparse-substrate repertoire queries *outside* the partition search, which would need the reduced-dimension factor representation, not just the inference.
...
- **B12** — `pyphi.Sweep` cartesian batch driver. *Promoted to `2.0` (see dashboard).* Lazy `Sweep(substrate).over_states().over_subsets().over_formalisms([...]).compute()` → one tidy MultiIndexed DataFrame (φ, n_distinctions, Σφ_r, version) with raw results in `.results` and a `ConfigSnapshot`/seed per row. Co-design the row schema with P14d `to_pandas`.
...
- **B15** — `result.diff(a, b)`. *Landed (2026-06-16); see the dashboard row + archive.* Structured `ResultDiff` (Δφ, genuine-MIP-change vs tie-reshuffle, distinctions/relations gained-lost keyed by mechanism) composing `ConfigSnapshot.diff`; `Displayable` + `to_pandas()` on every top-level result.
