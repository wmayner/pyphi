...
| Selection-margin reporting | ✅ landed | 7 | IIT 4.0 SIA reports `partition_margin` (normalized-φ gap to the best competing system partition), per-direction specified-state margins (second-best ii retained on `StateSpecification`), and `effectively_tied` (any margin within `precision` of zero) — surfaced via `explain()`, the FULL-verbosity card, `to_pandas`, and serialization. Extended to the exclusion competition (the theory's third selection): `Complex.exclusion_margin` (φₛ gap to the best overlapping rival the complex beat — beaten-rivals semantics; higher-φₛ shadows carried by the recursive cascade never enter) + `Complex.effectively_tied`, derived from exclusion records at both doors (micro `substrate.complexes` and macro grain search), on the display card and `to_pandas`; `exclusion_records` identity fix so same-footprint rival grains are recorded; `docs/tutorials/recursive-exclusion.md` demonstrates shadows + margins. IIT 3.0/AC deferred (different selection structure). Concrete slice of N23. Specs: `2026-07-07-substrate-parameter-landscapes.md` §7, `2026-07-09-grain-selection-margin-design.md` |
