...
| P15 | 🟡 partial | 5 | Import-surface cleanup landed (eager submodule walk → explicit registry imports + PEP-562 lazy `__getattr__`, guarded by a registry-contents test). Remaining: jsonify→msgspec, test reorg, docstring sweep, pandas extend (display → B21), PR triage, changelog condense |
...
**jsonify → msgspec serialization — new `pyphi.serialize` package built and validated (branch `serialize-msgspec`, pending merge + jsonify retirement).** A typed `msgspec`-based serializer replaces the custom `pyphi/jsonify.py` JSON layer: one frozen `msgspec.Struct` per result type in a tag-discriminated union (`serialize/schema.py`), centralized `to_schema`/`from_schema` converters keyed by type (`serialize/convert.py`) replacing the scattered `to_json`/`from_json` hooks, numpy arrays stored as their `.npy` bytes (`serialize/arrays.py`; base64 in JSON, raw in msgpack, `allow_pickle=False`), and one `dumps`/`loads`/`dump`/`load` API over a single top-level `format_version` with both JSON and msgpack from one schema. The cause-effect-structure schema is **normalized**: distinctions are stored once in a table and each relation references its members by index, removing the dominant redundancy of embedding every distinction in every relation. Tie-peer graphs (RIA, SIA, AcRIA, state-spec, CausalLink) are reconstructed on decode. Validated two ways: ~94 synthetic round-trip tests (every type, JSON + msgpack) and an **equivalence test** that loads each jsonify-format reference fixture with the legacy serializer and asserts the new serializer round-trips it (all 27 live result-object fixtures pass both formats; fixtures referencing classes removed in the 2.0 refactor are skipped). Measured compaction on the rule154 phi-structure: 1.31 MB → 56 KB JSON (23×), 43 KB msgpack (31×). Per a deliberate strategy choice, the old jsonify-format goldens are kept as the trusted reference set (not regenerated in place), so `jsonify` and `serialize` currently coexist. **Remaining to retire jsonify:** rewire the call-sites (`substrate.py` loader), remove `pyphi/jsonify.py` and the now-dead `to_json`/`from_json` hooks + tie-peer contextvars, and replace the legacy-serialization tests; ship a standalone old-format → new-format converter for users' saved results. *(Config-as-Struct and domain-level generative-relations compression stay deferred.)*

**Remaining P15:** finish the jsonify retirement (above); mirror `pyphi/` structure in `test/` + split mixed test files; **docstring sweep** (rewrite to final-state voice; remove planning-artifact leaks like `pre-P11.9` and project-stage markers); extend `to_pandas` (object-display unification is **B21**, scheduled pre-freeze in Wave 2); open-PR triage (#114/#116/#130/#134/#117); condense changelog fragments to first-encounter voice.
