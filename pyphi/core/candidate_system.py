...
class CandidateSystem:
    ...
    # ---- formalism queries ----
    #
    # Operations whose math depends on the active formalism (find_mip, sia,
    # cause/effect/phi_*_mip, find_mice, mic/mie/phi_max, concept,
    # distinction, all_distinctions, evaluate_partition,
    # _find_mip_single_state) are NOT methods on CandidateSystem. They live
    # as free functions in :mod:`pyphi.formalism`. The grammar of
    # ``formalism.find_mip(cs, ...)`` reflects the architectural reality:
    # the formalism is the agent making the policy decision; the candidate
    # system is the object it operates on.
    #
    # See ``test_core_layering.test_candidate_system_does_not_import_formalism``.
