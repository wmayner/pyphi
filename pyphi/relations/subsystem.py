...
class Subsystem:
    ...
    def find_mip(self, direction, mechanism, purview, maxp=0, get_ties=False):
    ...
    def find_mice(self, direction, mechanism, purviews=False, maxp=0):
        ...
        if direction == Direction.CAUSE:
            MIC = MaximallyIrreducibleCause(max_mip)
            if max_mip.phi>0:
                MIC._partlist = max_mip._partlist
            return MIC
        elif direction == Direction.EFFECT:
            MIE = MaximallyIrreducibleEffect(max_mip)
            if max_mip.phi>0:
                MIE._partlist = max_mip._partlist
            return MIE
        return validate.direction(direction)
