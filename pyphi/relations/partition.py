...
def mip_partitions(mechanism, purview, node_labels=None, relationsV=False, relationsBI=False):
    ...
    if relationsV:
        func = partition_types['RTRI']
    if relationsBI:
        func = partition_types['BI']
...
def rwedge_partitions(mechanism, purview, node_labels=None):
    ...
    def valid(factoring):
        ...
        return (
            (numerator[0] or denominator[0]) and
            (numerator[1] or denominator[1]) and
            (not numerator[0] or not numerator[1]) or
             (denominator[0] and denominator[1])
        )
