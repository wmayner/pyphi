...
@pytest.mark.parametrize(
    ...
    zip(
        TEST_DATA,
        [0.0, 0.75, 0.75, 0.5, 0.0, 0.266667, 0.266667, 0.5, 0.5, 0.0,],
...
def test_hamming_emd(pq, answer):
    assert np.allclose(metrics.hamming_emd(*pq), answer)
...
def test_l1_metrics(pq, answer):
    assert np.allclose(metrics.l1(*pq), answer)
...
def test_entropy_difference(pq, answer):
    result = metrics.entropy_difference(*pq)
    if np.isnan(answer):
...
def test_kld(pq, answer):
    assert np.allclose(metrics.kld(*pq), answer)
...
def test_psq2(pq, answer):
    assert np.allclose(metrics.psq2(*pq), answer)
...
def test_information_density(pq, answer):
    assert np.allclose(metrics.information_density(*pq), answer)
...
def test_intrinsic_difference(pq, answer):
    assert np.allclose(metrics.intrinsic_difference(*pq), answer)
...
def test_absolute_intrinsic_difference(pq, answer):
    assert np.allclose(metrics.absolute_intrinsic_difference(*pq), answer)
