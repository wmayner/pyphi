...
def test_expand_repertoire():
    ...
    assert np.all(abs(A.expand_cause_repertoire(BCD).flatten(order='F') -
    ...
    assert np.all(abs(A.expand_cause_repertoire(ABCD).flatten(order='F') -
                  np.array([1/12 if i < 12 else 0 for i in range(16)])) < EPSILON)
    assert np.all(abs(A.expand_effect_repertoire(ABCD).flatten(order='F') -
